<?php
	...
	include("../../mapco_shop_de/functions/shop_OrderSumGet.php");
	...
	if ( isset( $_SESSION["id_language"] ) && $_SESSION["id_language"] != "" )
	{
		//GET LANG_CODE
		$res_lang = q("SELECT * FROM cms_languages WHERE id_language = ".$_SESSION["id_language"], $dbweb, __FILE__, __LINE__);
		...
		{
			$row_lang = mysqli_fetch_assoc( $res_lang );
			$lang = $row_lang["code"];
		}
	}
	else
	...
	$languages = array("de", "en", "es", "fr", "it", "pl", "ru", "zh");
	...
	for ($k=0; $k<sizeof($order); $k++)
	{
		...
		if ( $order[$k]["payments_type_id"] != 0 )
		{
			$res = q("SELECT * FROM shop_payment_types WHERE id_paymenttype = " . $order[$k]["payments_type_id"] . ";", $dbshop, __FILE__, __LINE__);
			if ( mysqli_num_rows( $res ) == 1 )
			{
				$shop_payment_types = mysqli_fetch_array( $res );
				$payment_title[$k] = $shop_payment_types["title"];
			}
			else
			{
				$payment_title[$k]="";
	...
	// setting gewerblich by $_POST (mainly for testing reasons)
	if ( isset( $_POST['gewerblich'] ) and $_POST['gewerblich'] == 1 )
	{
		$ordertype="business"; 
		$gewerblich=true; 
	}
	
	if ( isset( $_POST['gewerblich'] ) and $_POST['gewerblich'] == 0 )
	{
		$ordertype="customer";
		$gewerblich=false; 
	}
...
//		$order_site_id=$shop[$parentshop_id ]["site_id"];
	...
	$order_sum = OrderSumGet($order[0]["id_order"]);
...
//		if ($in_orders=='') $in_orders.=$order[$j]["id_order"]; else $in_orders.=", ".$order[$j]["id_order"];
	...
	while ($row_order_items=mysqli_fetch_array($res_order_items))
	{
		...
		//ITEM DESCRIPTION
		$res_items_desc=q("SELECT * FROM shop_items_".$lang." WHERE id_item = ".$row_order_items["item_id"].";", $dbshop, __FILE__, __LINE__);
		$row_items_desc=mysqli_fetch_array($res_items_desc);
		
		//MPN, COS
		$res_items_MPN=q("SELECT MPN, COS, article_id FROM shop_items WHERE id_item = ".$row_order_items["item_id"].";", $dbshop, __FILE__, __LINE__);
...
//		else
		...
		$xmldata.="			<OrderItemDescLong><![CDATA[".$row_items_desc["short_description"]."]]></OrderItemDescLong>\n";
		...
		$xmldata.="			<OrderItemExchangeRateToEUR>".$order_sum ["OrderItems"] [$row_order_items["id"]] ["ExchangeRate"]."</OrderItemExchangeRateToEUR>\n";
		
		//ARTIKEL EINZELPREIS
		$xmldata.="			<orderItemPriceGrossFC>".number_format($order_sum ["OrderItems"] [$row_order_items["id"]] ["GrossFC"], 2,",","")."</orderItemPriceGrossFC>\n";
		$xmldata.="			<orderItemPriceGross>".number_format($order_sum ["OrderItems"] [$row_order_items["id"]] ["GrossEUR"], 2,",","")."</orderItemPriceGross>\n";
		$xmldata.="			<orderItemPriceNetFC>".number_format($order_sum ["OrderItems"] [$row_order_items["id"]] ["NetFC"], 2,",","")."</orderItemPriceNetFC>\n";
		$xmldata.="			<orderItemPriceNet>".number_format($order_sum ["OrderItems"] [$row_order_items["id"]] ["NetEUR"], 2,",","")."</orderItemPriceNet>\n";
		$xmldata.="			<orderItemPriceCOS_M>".number_format($cos_m, 2,",",".")."</orderItemPriceCOS_M>\n";
		$xmldata.="			<orderItemPriceTaxFC>".number_format($order_sum ["OrderItems"] [$row_order_items["id"]] ["TaxFC"], 2,",","")."</orderItemPriceTaxFC>\n";
		$xmldata.="			<orderItemPriceTax>".number_format($order_sum ["OrderItems"] [$row_order_items["id"]] ["TaxEUR"], 2,",","")."</orderItemPriceTax>\n";

		//ARTIKEL EINZELPREIS OHNE COLLATERAL
		$xmldata.="			<orderItemPriceWoCollateralGrossFC>".number_format($order_sum ["OrderItemsCollateral"] [$row_order_items["id"]] ["GrossFC"], 2,",","")."</orderItemPriceWoCollateralGrossFC>\n";
		$xmldata.="			<orderItemPriceWoCollateralGross>".number_format($order_sum ["OrderItemsCollateral"] [$row_order_items["id"]] ["GrossEUR"], 2,",","")."</orderItemPriceWoCollateralGross>\n";
		$xmldata.="			<orderItemPriceWoCollateralNetFC>".number_format($order_sum ["OrderItemsCollateral"] [$row_order_items["id"]] ["NetFC"], 2,",","")."</orderItemPriceWoCollateralNetFC>\n";
		$xmldata.="			<orderItemPriceWoCollateralNet>".number_format($order_sum ["OrderItemsCollateral"] [$row_order_items["id"]] ["NetEUR"], 2,",","")."</orderItemPriceWoCollateralNet>\n";
		$xmldata.="			<orderItemPriceWoCollateralTaxFC>".number_format($order_sum ["OrderItemsCollateral"] [$row_order_items["id"]] ["TaxFC"], 2,",","")."</orderItemPriceWoCollateralTaxFC>\n";
		$xmldata.="			<orderItemPriceWoCollateralTax>".number_format($order_sum ["OrderItemsCollateral"] [$row_order_items["id"]] ["TaxEUR"], 2,",","")."</orderItemPriceWoCollateralTax>\n";

		//ARTIKEL EINZEL COLLATERAL
		$xmldata.="			<orderItemCollateralPriceGrossFC>".number_format($order_sum ["OrderItemsWoCollateral"] [$row_order_items["id"]] ["GrossFC"], 2,",","")."</orderItemCollateralPriceGrossFC>\n";
		$xmldata.="			<orderItemCollateralPriceGross>".number_format($order_sum ["OrderItemsWoCollateral"] [$row_order_items["id"]] ["GrossEUR"], 2,",","")."</orderItemCollateralPriceGross>\n";
		$xmldata.="			<orderItemCollateralPriceNetFC>".number_format($order_sum ["OrderItemsWoCollateral"] [$row_order_items["id"]] ["NetFC"], 2,",","")."</orderItemCollateralPriceNetFC>\n";
		$xmldata.="			<orderItemCollateralPriceNet>".number_format($order_sum ["OrderItemsWoCollateral"] [$row_order_items["id"]] ["NetEUR"], 2,",","")."</orderItemCollateralPriceNet>\n";
		$xmldata.="			<orderItemCollateralPriceTaxFC>".number_format($order_sum ["OrderItemsWoCollateral"] [$row_order_items["id"]] ["TaxFC"], 2,",","")."</orderItemCollateralPriceTaxFC>\n";
		$xmldata.="			<orderItemCollateralPriceTax>".number_format($order_sum ["OrderItemsWoCollateral"] [$row_order_items["id"]] ["TaxEUR"], 2,",","")."</orderItemCollateralPriceTax>\n";

		//BESTELLPOSITION SUMME
		$xmldata.="			<orderItemTotalGrossFC>".number_format($order_sum ["OrderPositions"] [$row_order_items["id"]] ["GrossFC"], 2,",","")."</orderItemTotalGrossFC>\n";
		$xmldata.="			<orderItemTotalGross>".number_format($order_sum ["OrderPositions"] [$row_order_items["id"]] ["GrossEUR"], 2,",","")."</orderItemTotalGross>\n";
		$xmldata.="			<orderItemTotalNetFC>".number_format($order_sum ["OrderPositions"] [$row_order_items["id"]] ["NetFC"], 2,",","")."</orderItemTotalNetFC>\n";
		$xmldata.="			<orderItemTotalNet>".number_format($order_sum ["OrderPositions"] [$row_order_items["id"]] ["NetEUR"], 2,",","")."</orderItemTotalNet>\n";
		$xmldata.="			<orderItemTotalTaxFC>".number_format($order_sum ["OrderPositions"] [$row_order_items["id"]] ["TaxFC"], 2,",","")."</orderItemTotalTaxFC>\n";
		$xmldata.="			<orderItemTotalTax>".number_format($order_sum ["OrderPositions"] [$row_order_items["id"]] ["TaxEUR"], 2,",","")."</orderItemTotalTax>\n";

		//BESTELLPOSITION OHNE COLLATERAL
		$xmldata.="			<orderItemTotalWoCollateralGrossFC>".number_format($order_sum ["OrderPositionsWoCollateral"] [$row_order_items["id"]] ["GrossFC"], 2,",","")."</orderItemTotalWoCollateralGrossFC>\n";
		$xmldata.="			<orderItemTotalWoCollateralGross>".number_format($order_sum ["OrderPositionsWoCollateral"] [$row_order_items["id"]] ["GrossEUR"], 2,",","")."</orderItemTotalWoCollateralGross>\n";
		$xmldata.="			<orderItemTotalWoCollateralNetFC>".number_format($order_sum ["OrderPositionsWoCollateral"] [$row_order_items["id"]] ["NetFC"], 2,",","")."</orderItemTotalWoCollateralNetFC>\n";
		$xmldata.="			<orderItemTotalWoCollateralNet>".number_format($order_sum ["OrderPositionsWoCollateral"] [$row_order_items["id"]] ["NetEUR"], 2,",","")."</orderItemTotalWoCollateralNet>\n";
		$xmldata.="			<orderItemTotalWoCollateralTaxFC>".number_format($order_sum ["OrderPositionsWoCollateral"] [$row_order_items["id"]] ["TaxFC"], 2,",","")."</orderItemTotalWoCollateralTaxFC>\n";
		$xmldata.="			<orderItemTotalWoCollateralTax>".number_format($order_sum ["OrderPositionsWoCollateral"] [$row_order_items["id"]] ["TaxEUR"], 2,",","")."</orderItemTotalWoCollateralTax>\n";

		//BESTELLPOSITION COLLATERAL
		$xmldata.="			<orderItemTotalCollateralPriceGrossFC>".number_format($order_sum ["OrderPositionsCollateral"] [$row_order_items["id"]] ["GrossFC"], 2,",","")."</orderItemTotalCollateralPriceGrossFC>\n";
		$xmldata.="			<orderItemTotalCollateralPriceGross>".number_format($order_sum ["OrderPositionsCollateral"] [$row_order_items["id"]] ["GrossEUR"], 2,",","")."</orderItemTotalCollateralPriceGross>\n";
		$xmldata.="			<orderItemTotalCollateralPriceNetFC>".number_format($order_sum ["OrderPositionsCollateral"] [$row_order_items["id"]] ["NetFC"], 2,",","")."</orderItemTotalCollateralPriceNetFC>\n";
		$xmldata.="			<orderItemTotalCollateralPriceNet>".number_format($order_sum ["OrderPositionsCollateral"] [$row_order_items["id"]] ["NetEUR"], 2,",","")."</orderItemTotalCollateralPriceNet>\n";
		$xmldata.="			<orderItemTotalCollateralPriceTaxFC>".number_format($order_sum ["OrderPositionsCollateral"] [$row_order_items["id"]] ["TaxFC"], 2,",","")."</orderItemTotalCollateralPriceTaxFC>\n";
		$xmldata.="			<orderItemTotalCollateralPriceTax>".number_format($order_sum ["OrderPositionsCollateral"] [$row_order_items["id"]] ["TaxEUR"], 2,",","")."</orderItemTotalCollateralPriceTax>\n";
		...
		$res_article_image = q("SELECT * FROM cms_articles_images WHERE article_id = ".$row_items_MPN["article_id"], $dbweb, __FILE__, __LINE__);
		if ( mysqli_num_rows( $res_article_image ) > 0 )
		{
			...
			{
				$xmldata .= "			<ItemPicture imageformat_id='".$row_image_files["imageformat_id"]."'>\n";
				$xmldata .= "				<ItemPictureFileID>".$row_image_files["id_file"]."</ItemPictureFileID>\n";
		...
		for ($k=0; $k<sizeof($order); $k++)
		{
			...
			$xmldata.="		<shipping_netFC><![CDATA[".number_format($order_sum[$order[$k]["id_order"]]["ShippingCosts"] ["NetFC"], 2,",","")."]]></shipping_netFC>\n";
			$xmldata.="		<shipping_costsFC><![CDATA[".number_format($order_sum[$order[$k]["id_order"]]["ShippingCosts"] ["GrossFC"], 2,",","")."]]></shipping_costsFC>\n";
	/*
	...
	*/
			...
			{
				$xmldata.="		<shipping_costs><![CDATA[".number_format($order_sum[$order[$k]["id_order"]]["ShippingCosts"] ["GrossEUR"], 2,",","")."]]></shipping_costs>\n";
				$xmldata.="		<shipping_net><![CDATA[".number_format($order_sum[$order[$k]["id_order"]]["ShippingCosts"] ["NetEUR"], 2,",","")."]]></shipping_net>\n";
			}
...
//	$exchangerates[$row_order_items["order_id"]]
	...
	/*
		...
		*/
		{
			$xmldata.="	<shippingCostsGrossFC>".number_format($order_sum["ShippingCosts"] ["GrossFC"], 2,",","")."</shippingCostsGrossFC>\n";
			$xmldata.="	<shippingCostsGross>".number_format($order_sum["ShippingCosts"] ["GrossEUR"], 2,",","")."</shippingCostsGross>\n";
			$xmldata.="	<shippingCostsNetFC>".number_format($order_sum["ShippingCosts"] ["NetFC"], 2,",","")."</shippingCostsNetFC>\n";
			$xmldata.="	<shippingCostsNet>".number_format($order_sum["ShippingCosts"] ["NetEUR"], 2,",","")."</shippingCostsNet>\n";
			$xmldata.="	<shippingCostsTaxFC>".number_format($order_sum["ShippingCosts"] ["TaxFC"], 2,",","")."</shippingCostsTaxFC>\n";
			$xmldata.="	<shippingCostsTax>".number_format($order_sum["ShippingCosts"] ["TaxEUR"], 2,",","")."</shippingCostsTax>\n";
		//Item Total
			$xmldata.="	<orderItemsTotalGrossFC>".number_format($order_sum["OrderPositionsTotal"] ["GrossFC"], 2,",","")."</orderItemsTotalGrossFC>\n";
			$xmldata.="	<orderItemsTotalGross>".number_format($order_sum["OrderPositionsTotal"] ["GrossEUR"], 2,",","")."</orderItemsTotalGross>\n";
			$xmldata.="	<orderItemsTotalNetFC>".number_format($order_sum["OrderPositionsTotal"] ["NetFC"], 2,",","")."</orderItemsTotalNetFC>\n";
			$xmldata.="	<orderItemsTotalNet>".number_format($order_sum["OrderPositionsTotal"] ["NetEUR"], 2,",","")."</orderItemsTotalNet>\n";
		//	$xmldata.="	<orderItemsTotalCollateralGross>".number_format($orderItemsTotalCollateralGross, 2,",","")."</orderItemsTotalCollateralGross>\n";
		//	$xmldata.="	<orderItemsTotalCollateralNet>".number_format($orderItemsTotalCollateralNet, 2,",","")."</orderItemsTotalCollateralNet>\n";
			$xmldata.="	<orderItemsTotalTaxFC>".number_format($order_sum["OrderPositionsTotal"] ["TaxFC"], 2,",","")."</orderItemsTotalTaxFC>\n";
			$xmldata.="	<orderItemsTotalTax>".number_format($order_sum["OrderPositionsTotal"] ["TaxEUR"], 2,",","")."</orderItemsTotalTax>\n";
		//OrderTotal
			$xmldata.="	<orderTotalGrossFC>".number_format($order_sum["OrderTotal"] ["GrossFC"], 2,",","")."</orderTotalGrossFC>\n";
			$xmldata.="	<orderTotalGross>".number_format($order_sum["OrderTotal"] ["GrossEUR"], 2,",","")."</orderTotalGross>\n";
			$xmldata.="	<orderTotalNetFC>".number_format($order_sum["OrderTotal"] ["NetFC"], 2,",","")."</orderTotalNetFC>\n";
			$xmldata.="	<orderTotalNet>".number_format($order_sum["OrderTotal"] ["NetEUR"], 2,",","")."</orderTotalNet>\n";
			$xmldata.="	<orderTotalTaxFC>".number_format($order_sum["OrderTotal"] ["TaxFC"], 2,",","")."</orderTotalTaxFC>\n";
			$xmldata.="	<orderTotalTax>".number_format($order_sum["OrderTotal"] ["TaxEUR"], 2,",","")."</orderTotalTax>\n";
		//ORDER - REFUND
		/*
			$xmldata.="	<completeTotalGrossFC>".number_format($orderTotalGrossFC-$refund_total_grossFC, 2,",","")."</completeTotalGrossFC>\n";
			$xmldata.="	<completeTotalGross>".number_format($orderTotalGross-$refund_total_grossEUR, 2,",","")."</completeTotalGross>\n";
