<?php
	...
	$required=array("SELECTOR_id_returnitem" =>"numericNN");
	check_man_params($required);
	
	//CHECK IF ENTRY EXISTS
	$res_check=q("SELECT * FROM shop_returns_items WHERE id_returnitem = ".$_POST["SELECTOR_id_returnitem"].";", $dbshop, __FILE__, __LINE__);
	if (mysqli_num_rows($res_check)==0)
	{
		show_error(9782, 7, __FILE__, __LINE__, "ReturnItemID: ".$_POST["SELECTOR_id_returnitem"]);
		exit;
	}
	$returnsitem=mysqli_fetch_array($res_check);
	
	//GET RETURN FOR ORDERID
	$res_check=q("SELECT * FROM shop_returns2 WHERE id_return = ".$returnsitem["return_id"].";", $dbshop, __FILE__, __LINE__);
	if (mysqli_num_rows($res_check)==0)
	{
		show_error(9778, 7, __FILE__, __LINE__, "ReturnID: ".$returnsitem["return_id"]);
		exit;
	}
	...
	$nvp="";
	$selector="";
	$res_struct=q("SHOW COLUMNS FROM shop_returns_items;", $dbshop, __FILE__, __LINE__);
	...
	if ($nvp!="" && $selector!="")
	{
		$sql="UPDATE shop_returns_items SET ".$nvp." WHERE ".$selector.";";
		...
		$xml.='</data>';
			...
			//SAVE EVENT
		$responseXml = post(PATH."soa2/", array("API" => "shop", "APIRequest" => "OrderEventSet", "order_id" => $returns["order_id"], "eventtype_id" => 27, "data" => $xml));
