...
// EINGABEN LEEREN
	...
	$sql.="state, platform, userid, buyername, MPU, articlegroup, quantity, transactionID, invoiceID, rAction, rReason, rReason_detail, exchange_MPU, exchange_quantity, date_exchange_sent, date_order, date_announced, date_return, date_refund, date_refund_reshipment, date_r_IDIMS, ebay_demand_closing1, ebay_demand_closing2, ebay_fee_refund, refund, refund_reshipment, firstmod, firstmod_user, lastmod, lastmod_user";
	$sql.=") VALUES(";
	$sql.="'".$_POST["state"]."', '".$_POST["platform"]."', '".$userid."', '".$_POST["buyerName"]."', '".$_POST["MPU"]."', '".$_POST["article_group"]."','".$_POST["quantity"]."', '".$_POST["transactionID"]."', '".$_POST["invoiceID"]."', '".$_POST["rAction"]."', '".$_POST["rReason"]."', '".$_POST["rReason_detail"]."' , '".$_POST["exchange_MPU"]."', '".$_POST["exchange_quantity"]."', '".dateConverter($_POST["date_exchange_sent"])."', '".dateConverter($_POST["date_order"])."', '".dateConverter($_POST["date_announced"])."', '".dateConverter($_POST["date_return"])."', '".dateConverter($_POST["date_refund"])."', '".dateConverter($_POST["date_refund_reshipment"])."', '".dateConverter($_POST["date_r_IDIMS"])."', '".dateConverter($_POST["date_demandEbayClosing1"])."', '".dateConverter($_POST["date_demandEbayClosing2"])."', '".$_POST["ebayFeeRefundOK"]."', '".$refund."', '".$refund_reshipment."' ,'".time()."', '".$_SESSION["id_user"]."', '".time()."', '".$_SESSION["id_user"]."'";
	$sql.=");";
