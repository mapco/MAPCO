<?php
	...
	$result=q("SELECT combined_with FROM shop_orders WHERE id_order=".$id_order.";", $dbshop, __FILE__, __LINE__);
	if (mysqli_num_rows($result)==0)
	{ 
		show_error(9762, 7, __FILE__, __LINE__, "OrderID:".$id_order);
		exit;
	}
	else
	{
		...
		$sql = 'SELECT '.$select_fields.' FROM shop_orders WHERE';
	
		if($row['combined_with'] < 1)
		{ 
			$singleorder=true;
		...
		{
			$singleorder=false;
			$sql .= " id_order=".$row['combined_with'];
		}
		
		//$sql .= " AND s.id_shippingtype=o.shipping_type_id";
		//GET ORDERdata
		$result=q($sql, $dbshop, __FILE__, __LINE__);
		$order=mysqli_fetch_assoc($result);
		$order_ids[0] = $order['id_order'];
	}

	if (!$singleorder)
	{
		$sql = 'SELECT id_order FROM shop_orders WHERE combined_with='.$order['combined_with'].' AND NOT id_order='.$order['combined_with'].';';
		$result=q($sql, $dbshop, __FILE__, __LINE__);
		while ($row=mysqli_fetch_assoc($result))
		{
			$order_ids[sizeof($order)]=$row['id_order'];
		}
	}
	...
	$TmpOrderTotalGrossFC=0;
	$TmpOrderTotalGross=0;
	$TmpOrderTotalNetFC=0;
	$TmpOrderTotalNet=0;
	$TmpOrderTotalTaxFC=0;
	$TmpOrderTotalTax=0;
	...
	$sql = "SELECT id, order_id, foreign_TransactionID, item_id, amount, price, exchange_rate_to_EUR, netto, Currency_Code FROM shop_orders_items WHERE order_id IN (".implode(", ", $order_ids).")";
	...
	$order['OrderTotalGrossFC']=number_format($TmpOrderTotalGrossFC, 2,",",".");
	$order['OrderTotalGross']=number_format($TmpOrderTotalGross, 2,",",".");
	$order['OrderTotalNetFC']=number_format($TmpOrderTotalNetFC, 2,",",".");
	$order['OrderTotalNet']=number_format($TmpOrderTotalNet, 2,",",".");
	$order['OrderTotalTaxFC']=number_format($TmpOrderTotalTaxFC, 2,",",".");
	$order['OrderTotalTax']=number_format($TmpOrderTotalTax, 2,",",".");
