...
//		echo $y+$lh.'<br />';
		...
		//get OENr
		$OENr="";
		$results2=q("SELECT * FROM t_203 WHERE ArtNr='".$ArtNr."';", $dbshop, __FILE__, __LINE__);
		if ( mysqli_num_rows($results2)>0 )
		{
			$row2=mysqli_fetch_array($results2);
			if ( $row2["OENr"]!=0 )	$OENr=$row2["OENr"];
		}
		$x[1]=35.9;
		$lines[2]=$pdf->getStringWidth($OENr);
		$text[1]=$vehicles;
		...
		if ($y>250)
		{
			...
			$pdf->SetXY(145.9, $y);
			$pdf->MultiCell(57.5, 6, utf8_decode(t("Vergleichsnummer", __FILE__, __LINE__, $_POST["lang"])), 1, 'C', 0);
		}
		$y=$pdf->GetY();
...
//		array_multisort($lines, );
		...
		$pdf->SetXY(145.9, $y);
		$pdf->MultiCell(57.5, $nextY-$y, $OENr , 1, 'C', 0);
		$pdf->SetXY(0, $nextY);
	}
	...
	echo '<a target="_blank" href="'.PATH.'test.pdf">Download PDF</a>';
