...
$_POST["id_account"]=1;
$_POST["from"]=time()-50000; // 6 stunden
//$_POST["from"]=1370046017;
$_POST["to"]=time();
//$_POST["to"]=1370118017;
...
//exit;
	...
	for($i=0; isset($xml->OrderArray[0]->Order[$i]); $i++)
	{
		...
		foreach($xml->OrderArray[0]->Order[$i]->TransactionArray[0]->Transaction as $transaction)
		{
			...
			$trans_check_res=q("SELECT * FROM ebay_orders_items_test WHERE TransactionID='".$transactionID[$k]."';", $dbshop, __FILE__, __LINE__);
			if (mysql_num_rows($trans_check_res)>0)
			{
				$trans_check_row[$k]=mysql_fetch_array($trans_check_res);
				$transaction_old[$k]["TransactionID"]=$transactionID[$k];
				//GET ACCORDING ORDER
				$order_check_res=q("SELECT * FROM ebay_orders_test WHERE OrderID = '".$trans_check_row[$k]["OrderID"]."';", $dbshop, __FILE__, __LINE__);
				if (mysql_num_rows($order_check_res)>0)
				{
					$row=mysql_fetch_array($order_check_res);
	...
	//echo "insert new transaction<br/>";
					...
					$query="INSERT INTO ebay_orders_items_test (";
				...
				else
				{
					...
					if (!$t_equals)
					{
						...
						$query="UPDATE ebay_orders_items_test SET ";
						for($l=0; $l<sizeof($t_sql[$k]); $l++)
						{
...
//ORDERS ************************************************************************************************************************
			...
			{ //Upadate Order
				...
				//echo "UPDATE ORDER# ".$firstorder_old."<br />";
					...
					$query="UPDATE ebay_orders_test SET ";
					...
					$res=q("SELECT * FROM ebay_orders_test WHERE OrderID = '".$OrderID."';",$dbshop, __FILE__, __LINE__);
					$row=mysql_fetch_array($res);
			
				
				//SHOP ORDER UPDATE
					$varField=array();
					$varField["API"]="crm_test";
				...
				//ALLE ANDEREN ALTEN ORDERS LÖSCHEN
					...
					for ($k=0; $k<sizeof($old_orderids);$k++)
					{
						...
						{ 
							q("DELETE FROM ebay_orders_test WHERE OrderID = '".$old_orderids[$k]."';", $dbshop, __FILE__, __LINE__);
	...
	//echo "ADD ORDER: ".$OrderID."<br />";
				...
				$query="INSERT INTO ebay_orders_test (";
				for($j=0; $j<sizeof($sql); $j++)
				{
					$query.=$sql[$j]["name"];
					if ( ($j+1)<sizeof($sql) ) $query.=", ";
				...
				$id_order=mysql_insert_id($dbshop);
				$varField["API"]="crm_test";
				$varField["Action"]="import_ebayOrderData";
				$varField["mode"]="add";
