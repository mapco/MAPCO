<?php
	if ( !isset($_POST["id_accountsite"]) )
	{
		...
		echo '		<shortMsg>Accountseite nicht gefunden.</shortMsg>'."\n";
		echo '		<longMsg>Es muss eine Accountseiten-ID übermittelt werden, damit der Service weiß, welche Auktion aktualisiert werden soll.</longMsg>'."\n";
		echo '	</Error>'."\n";
		echo '</OrdersUpdateResponse>'."\n";
		exit;
	}
	...
	if ( mysqli_num_rows($results)==0 )
	{
		...
		echo '		<longMsg>Die angegebene Ebay-Accountseite konnte nicht gefunden werden. Die Account-ID scheint es nicht zu geben.</longMsg>'."\n";
		echo '	</Error>'."\n";
		echo '</OrdersUpdateResponse>'."\n";
		exit;
	}
...
//	$requestXmlBody .= '  <NumberOfDays>7</NumberOfDays>';
	...
	//submit auction
	$response = post(PATH."soa/", array("API" => "ebay", "Action" => "EbaySubmit", "Call" => "GetOrders", "id_accountsite" => $_POST["id_accountsite"], "request" => $requestXmlBody));
...
//	echo 'Has More Orders: '.$xml->HasMoreOrders[0].'</p>';
	...
	for($i=0; isset($xml->OrderArray[0]->Order[$i]); $i++)
	{
		...
		$sql[$j]["value"]=$account["id_account"];
		$j++;
		...
		foreach($xml->OrderArray[0]->Order[$i]->TransactionArray[0]->Transaction as $transaction)
		{
			...
			$t_sql[$k][$l]["value"]=$account["id_account"];
			$l++;
	...
	//OREDERS	
		...
		for ($k=0; $k<sizeof($t_sql); $k++)
		{
			$row=mysqli_fetch_array($t_results[$k]);
