...
//print_r($PN_data);
	...
	if($PN_data["payment_status"]=="Completed")
	{
		...
		//Zahldaten in die shop_orders schreiben
		//UPDATE SHOP_ORDER PAYMENTSTATUS
	//	update_PaymentStatus ($PN_data["Order_ID"], $id_PN, $PN_data["payment_status"], $PN_data["payment_date"]);	
	...
	if($PN_data["payment_status"]=="Refunded")
	{
		...
		$insert_data["buyer_lastname"]=$PN_data["payer_lastname"];
		$insert_data["buyer_firstname"]=$PN_data["payer_firstname"];
		$insert_data["parentPaymentTransactionID"]=$PN_data["ParentTransactionID"];
	...
	//	$postfields["orderid"]=$PN_data["Order_ID"];
		...
		$responseXML = post(PATH."/soa2/", $postfields);
		mail("nputzing@mapco.de", "TEST", $responseXML);
		$use_errors = libxml_use_internal_errors(true);
		try
		{
			$response = new SimpleXMLElement($responseXML);
