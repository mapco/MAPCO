<?php
	...
	while ($row_order_notifications = mysqli_fetch_assoc($res_order_notifications))
	{
		...
		{
			$orderdata[]=$row_order_notifications;
		}
	...
	//ORDERDATA
	echo '<orderdata>'."\n";
	echo '<lastOrderDeposit>'.$lastOrderDepositFC.'</lastOrderDeposit>'."\n";
	echo '<lastOrderDepositEUR>'.$lastOrderDepositEUR.'</lastOrderDepositEUR>'."\n";
	...
	foreach ($orderdata as $id_PN => $data)
	{
		echo '<accounting>'."\n";
