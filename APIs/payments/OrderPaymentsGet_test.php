<?php
	...
	if (!$single)
	{
		$res_order=q("SELECT * FROM ".Shop_Order." WHERE id_order =".$_POST["orderid"], $dbshop, __FILE__, __LINE__);
		...
		if ($order[0]["combined_with"]>0)
		{
			...
			$res_orders=q("SELECT * FROM ".Shop_Order." WHERE id_order = ".$tmp_orderid.";", $dbshop, __FILE__, __LINE__);
			$order[0]=mysqli_fetch_array($res_orders);
			
			$orderids[]=$order[0]["id_order"];
			
			
			$res_orders=q("SELECT * FROM ".Shop_Order." WHERE combined_with = ".$tmp_orderid." AND NOT id_order = ".$tmp_orderid.";", $dbshop, __FILE__, __LINE__);
...
//	$lastOrderDepositEUR= false;
	...
	foreach ($transactions as $transactionID => $transactionsData)
	{
		...
		foreach ($transactionsData as $id_PN => $PN_Data)
		{
			...
			if ($PN_Data["notification_type"]==5)
			{
				$paymentsAccountingsEUR+=$PN_Data["accounting_EUR"];
			}
			if ($PN_Data["notification_type"]==1 && $PN_Data["accounting_EUR"]!=0)
			{
				$TransactionFee=$PN_Data["fee"];
				$TransactionExchangeRate=$PN_Data["exchange_rate_from_EUR"];
			}

			echo '</accounting>'."\n";
		}
		echo '<TransactionFee>'.$TransactionFee.'</TransactionFee>'."\n";
		echo '<TransactionExchangeRate>'.$TransactionExchangeRate.'</TransactionExchangeRate>'."\n";
		echo '</transaction>'."\n";
	}
	echo '<paymentsAccountingsEUR>'.$paymentsAccountingsEUR.'</paymentsAccountingsEUR>'."\n";
	echo '</paymentdata>'."\n";
