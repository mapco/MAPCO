<?php
include("PayPalCallerService.php");
...
for ($i=0; $i<sizeof($transactionIDs); $i++)
{
	...
	if ($field["ACK"]=="Success")
	{
		...
		$res=q("UPDATE shop_orders SET PayPal_TransactionState = '".mysqli_real_escape_string($dbshop, $field["PAYMENTSTATUS"])."', PayPal_PendingReason = '".mysqli_real_escape_string($dbshop, $pendingreason)."', PayPalTransactionStateDate = ".time()." WHERE PayPal_TransactionID = '".$transactionIDs[$i]."';", $dbshop, __FILE__, __LINE__);
	}
	else
	{
