...
//echo $responseXml;
	...
	//	echo "\n".$currency;
	//	echo $exchangerate_to_EUR[$currency];
	//	echo "<br />";
		$bestOfferBuyItNowForeignCurrency = number_format($xml->ItemBestOffersArray[0]->ItemBestOffers[$i]->Item[0]->BuyItNowPrice[0], 2,".", "");
	//	echo "<br />";
		$bestOfferBuyItNowEUR=($bestOfferBuyItNowForeignCurrency*(1/$exchangerate_to_EUR[$currency]));
//		$bestOfferBuyItNowEUR.='';
		$bestOfferBuyItNow = number_format((string)$bestOfferBuyItNowEUR, 2,".","");
	//	echo "<br />";
			...
			for($j=0; isset($xml->ItemBestOffersArray[0]->ItemBestOffers[$i]->BestOfferArray[$j]->BestOffer[0]->BestOfferID[0]); $j++)
			{
				...
				if ((time()-strtotime($Expiration))<169200) // 169200 => 47h
				{
					...
					else
					{
						...
						{
							if((time()-$row_buyings["CreatedDateTimestamp"])<48*3600) 
							{
								$respondToOffer=false;
		...
		//				echo "Preisvorschlag senden";
						...
						$bestOfferPrice[$k] = number_format($xml->ItemBestOffersArray[0]->ItemBestOffers[$i]->BestOfferArray[$j]->BestOffer[0]->Price[0], 2, ".", "");
						$bestOfferPrice[$k] = $bestOfferPrice[$k]*(1/$exchangerate_to_EUR[$currency]);
		...
		//			echo " red price: "; 
			...
			//		echo " discounted yellow: ";
					...
					{
						if ($price_red<$discounted_bestOfferBuyItNow)
						{
							$new_price_offer=$discounted_bestOfferBuyItNow;
						}
						elseif ($price_red<$bestOfferBuyItNow)
						{
							$new_price_offer=$price_red;
						}
						else
			...
			//		echo " NewPriceOffer ".$new_price_offer."<br>";
					...
					//check, ob Käuferpreisvorschlag über verkaufspreis liegt - > ablehnen
					if ($bestOfferBuyItNow < $bestOfferPrice[$j])
					{
						echo "ABLEHNUNG".$bestOfferBuyItNow."####".$bestOfferPrice[$j]."||||||||".$bestOfferBuyItNowEUR;
						$message='Der von Ihnen abgegebene Preisvorschlag liegt über dem Verkaufspreis. Daher wird Ihr Preisvorschlag abgelehnt. Haben Sie eventuell vergessen das Komma (,) im Preis einzugeben? Bitte beachten Sie zudem bei der Abgabe von Preisvorschlägen, dass diese pro Artikel gelten.';
											
						//PREISVORSCHLAG ABLEHNEN				
						$responseXml2 = post(PATH."soa/", array("API" => "ebay", "Action" => "RespondBestOffers", "id_account" => $_POST["account_id"], "BestOfferID" => $bestOfferID[$j], "ItemID" => $bestOfferItemID, "BestOfferAction" => "Decline", "message" => $message));
					}
					//Check, ob vorzuschlagender Preis == || > VK Preis is -> ablehenen	
					//Check, ob neuer Preisvorschlag über Ebay VK liegt
					elseif ($new_price_offer >= $bestOfferBuyItNow || $new_price_offer==0)
					{
						echo "GEGENVORSCHLAG über BuyItNow";
						...
						//PREISVORSCHLAG ABLEHNEN				
						$responseXml2 = post(PATH."soa/", array("API" => "ebay", "Action" => "RespondBestOffers", "id_account" => $_POST["account_id"], "BestOfferID" => $bestOfferID[$j], "ItemID" => $bestOfferItemID, "BestOfferAction" => "Decline"));
					...
					//Gegenvorschlag senden / Preisvorschlag annehmen
					{
						...
						{
							echo "ACCEPT newpriceoffer:".$new_price_offer." Bestoffer:" .$bestOfferPrice[$j]." BuyItNow:".$bestOfferBuyItNowEUR;
						...
						{
							echo "GEGENVORSCHLAG EUR:".$new_price_offer." ".$currency.": ".number_format($new_price_offer*$exchangerate_to_EUR[$currency],2,".","")."BuyItNow:".$bestOfferBuyItNowEUR;
