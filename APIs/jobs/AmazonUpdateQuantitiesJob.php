<?php
	...
	function amazon_sendrequest($url, $datapost=array())
	{
		...
		// create request
		echo $url = "https://".$host.$uri."?".$canonicalized_query."&Signature=".$signature;
	...
	$results = q("
		...
		WHERE id_account = 1;", $dbshop, __FILE__, __LINE__);
	...
	//submit quantities
	$request = "Action=SubmitFeed&FeedType=_POST_INVENTORY_AVAILABILITY_DATA_";
	$datapost=array();
...
//		q("UPDATE amazon_products SET lastquantityupdate=".time()." WHERE id_product=".$row["id_product"].";", $dbshop, __FILE__, __LINE__);
		...
		q("UPDATE amazon_products SET Quantity=".$Quantity.", lastquantityupdate=".time()." WHERE id_product=".$row["id_product"].";", $dbshop, __FILE__, __LINE__);
