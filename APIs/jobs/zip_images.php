...
//	include("../config.php");
	...
	if (mysqli_num_rows($results)>0)
	{
		while($row=mysqli_fetch_array($results))
		{
			...
			if (mysqli_num_rows($results2)>0)
			{
				...
				//	if ($_GET["format"]==1) $file[]='../../mapco_shop_de/files/'.bcdiv($row["id_file"], 1000, 0).'/'.$row["id_file"].'.jpg';
				//	else					$file[]='../../mapco_shop_de/files/'.bcdiv($row["id_file"], 1000, 0).'/'.$row["id_file"].'.jpg';
					$file[]='../files/'.bcdiv($row["id_file"], 1000, 0).'/'.$row["id_file"].'.jpg';
					$file_artnr[]=$row3["MPN"];
				}
			}
		}
	...
	echo 'Jobdauer: '.($stoptime-$starttime).'<br />';
?>