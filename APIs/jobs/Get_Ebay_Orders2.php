...
//if (!isset($_POST["from"])) 
$_POST["from"]=time()-12*3600; 
//$_POST["from"]=mktime(0,0,1,8,26,2013);
//$_POST["from"]=time()-20*3600; 
//if (!isset($_POST["to"]))
 $_POST["to"]=time();
//$_POST["to"]=mktime(23,59,59,8,26,2013);
...
//	$requestXmlBody .= '  <NumberOfDays>7</NumberOfDays>';
	...
	//submit auction
	$response = post(PATH."soa/", array("API" => "ebay", "Action" => "EbaySubmit", "Call" => "GetOrders", "id_accountsite" => 9, "request" => $requestXmlBody));
			...
			{ //Upadate Order
				...
				*/
				//SHOP ORDER UPDATE
					$varField=array();
					$varField["API"]="crm";
					$varField["Action"]="import_ebayOrderData2";
...
//COMBINE orders
	$res_shops=q("SELECT * FROM shop_shops WHERE shop_type = 2 AND account_id = ".$_POST["id_account"].";", $dbshop, __FILE__, __LINE__);
