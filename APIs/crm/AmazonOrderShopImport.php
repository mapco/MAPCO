...
//	get the amazon order by amazon order id
$data = array();
$data['from'] = 'amazon_orders';
$data['select'] = '*';
$data['where'] = "
	AmazonOrderId = '" . $post['AmazonOrderId'] . "'";
$amazon_order = SQLSelect($data['from'], $data['select'], $data['where'], 0, 0, 1, 'shop',  __FILE__, __LINE__);
...
if (strtotime($amazon_order["PurchaseDate"]) > 1394837999) {
	//	GET ORDERITEMS
		...
		$amazonAccountsSitesQuery = "
			...
			WHERE MarketplaceID = '" . $amazon_order["MarketplaceId"] . "'";
		...
		$amazonAccountssitesShopShopsQuery = "
			SELECT * 
			FROM shop_shops 
			WHERE account_id = '" . $amazonAccountsSites['account_id'] . "' AND shop_type = 3";
		$res_shop = q($amazonAccountssitesShopShopsQuery, $dbshop, __FILE__, __LINE__);
		...
		//	GET COUNTRY
		//	the ShippingAddressCountryCode does not empty
		$shopCountriesResult = q("
			SELECT * 
			FROM shop_countries 
			WHERE country_code = '" . $amazon_order["ShippingAddressCountryCode"] . "'", $dbshop, __FILE__, __LINE__);
		if (mysqli_num_rows($shopCountriesResult) == 0)
		{
			if (!empty($amazon_order["ShippingAddressCountryCode"])) {
				$errorShippingAddress = "
					Unknown Country Code = " . $amazon_order["ShippingAddressCountryCode"] . "<br />" . "AmazonOrderId=" . $amazon_order['AmazonOrderId'];
			} else {
				$errorShippingAddress = "
					Country Code is empty for AmazonOrderId = " . $amazon_order['AmazonOrderId'];
			}
			show_error(9817, 7,__FILE__, __LINE__, $errorShippingAddress);
			q("UPDATE amazon_orders 
				SET importShopStatus = 1
				WHERE AmazonOrderId = '" . $post["AmazonOrderId"] . "'", $dbshop, __FILE__, __LINE__ );
			exit;
		}
		$country = mysqli_fetch_assoc($shopCountriesResult);
		...
		if ($cms_user_id == 0) { 
			...
			$insert_field["user_salt"] = $salt;
			$insert_field["userrole_id"] = 5;
			$insert_field["language_id"] = $country['id_country'];
...
//			$fieldlist["Payments_TransactionID"]=$IPNs[0]["paymentTransactionID"];
            ...
            $fieldlist["Payments_Type"]="Vorkasse";
            $fieldlist["payments_type_id"]=2;
//			$fieldlist["Payments_TransactionState"]="Completed";
//			$fieldlist["Payments_TransactionID"]=$IPNs[0]["paymentTransactionID"];
            $fieldlist["bill_adr_id"]=$address_id;
            $fieldlist["firstmod"]=strtotime($amazon_order["PurchaseDate"]);
            $fieldlist["shipping_net"]=$shipping_costs_net;
		...
		//	REPLACE FANTASY SKUs
		$tempOrderItems=$orderItems;
		for($i=0; $i<sizeof($orderItems); $i++)
        {
            //	GET SHOP_ITEM_ID
			$MPN = $orderItems[$i]["SellerSKU"];
            if (strpos($MPN, "-") !== false) {
                $MPN = substr($MPN, 0, strpos($MPN, "-"));
            }
            $res_item = q("SELECT * FROM shop_items WHERE MPN = '" . $MPN . "'", $dbshop, __FILE__, __LINE__);
            if (mysqli_num_rows($res_item) == 0)
			{
				//	GET FANTASY MPN
				$amazon_bundles_results=q("SELECT * FROM amazon_products_bundles WHERE SKU='".mysqli_real_escape_string($dbshop, $orderItems[$i]["SellerSKU"])."';", $dbshop, __FILE__,__LINE__);
				if ( mysqli_num_rows($amazon_bundles_results) == 0 )
				{
					show_error(9877, 7, __FILE__, __LINE__, print_r($orderItems[$i], true));
					exit;
				}
				while( $amazon_bundles=mysqli_fetch_assoc($amazon_bundles_results) )
				{
					$nr=sizeof($tempOrderItems);
					$tempOrderItems[$nr]=$orderItems[$i];
					$tempOrderItems[$nr]["SellerSKU"]=$amazon_bundles["SellerSKU"];
					$tempOrderItems[$nr]["QuantityOrdered"]=$amazon_bundles["QuantityOrdered"];
					$tempOrderItems[$nr]["ItemPriceAmount"]=$amazon_bundles["ItemPriceAmount"];
				}
				unset($tempOrderItems[$i]);
            }
		}
        ...
        foreach ($orderItems as $OrderItemId => $OrderItem)
        {
			...
			$results=q("SELECT * FROM shop_items WHERE MPN='".$MPN."';", $dbshop, __FILE__, __LINE__);
		...
		{	
			...
			$OK = true;
            if ((string)$response->Ack[0] != "Success") {
				$showInfo = 'API ' . $fieldlist["API"] . '->' . $fieldlist["APIRequest"] . ': (OID: ' . $fieldlist["order_id"] . ') ' . "\n";
				$showInfo.= 'in Line ' . __LINE__ . "\n";
				$showInfo.= (string)$response . "\n";
				$showInfo.= $fieldlist . "\n";
				show_error(9797, 8, __FILE__, __LINE__, $showInfo);
                $OK = false;
	...
	$shop_orders_results=q("SELECT * FROM shop_orders WHERE id_order=".$order_id.";", $dbshop, __FILE__, __LINE__);
	$shop_orders=mysqli_fetch_assoc($shop_orders_results);
	if( $shop_orders["Payments_TransactionID"]!="" ) $OK=false;
