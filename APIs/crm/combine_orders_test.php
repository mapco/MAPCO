<?php
	...
	if ($_POST["mode"]=="scan")
	{
		...
		//get EBAY ORDERS
		$res_ebay=q("SELECT OrderID, ShippingAddressAddressID FROM ebay_orders_test WHERE account_id = ".$shop["account_id"].";", $dbshop, __FILE__, __LINE__);
		...
		for ($k=0; $k<sizeof($stati); $k++)
		{
			...
			$res=q("SELECT * FROM shop_orders_test WHERE shop_id = ".$_POST["shop_id"]." AND status_id = ".$stati[$k].";" , $dbshop, __FILE__, __LINE__);
		...
		$orders=array();
		$res=q("SELECT * FROM shop_orders_test WHERE shop_id = ".$_POST["shop_id"]." and combined_with > 0;", $dbshop, __FILE__, __LINE__);
		while ($row=mysqli_fetch_array($res))
		{
			{
		...
		foreach ($orders as $order => $orderline)
		{
			...
			foreach ($orderline as $id_order => $line)
			{
				if ($line["status_id"]!=$orders[$order][$order]["status_id"] )
				{
					...
					q("UPDATE shop_orders_test SET combined_with = 0 WHERE id_order = ".$id_order.";", $dbshop, __FILE__, __LINE__);
				}
		...
		while ( list ($key, $val) = each ($orders))
		{
			if ($val == 1) 
			{
				...
				q("UPDATE shop_orders_test SET combined_with = 0 WHERE id_order = ".$key.";", $dbshop, __FILE__, __LINE__);
			}
		}
	...
	if ($_POST["mode"]=="unset_combination")
	{
		...
		$res=q("SELECT * FROM shop_orders_test WHERE combined_with = ".$_POST["combined_with"].";", $dbshop, __FILE__, __LINE__);
		...
		q("UPDATE shop_orders_test SET combined_with = -1 WHERE combined_with = ".$_POST["combined_with"].";", $dbshop, __FILE__, __LINE__);
