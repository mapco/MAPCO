<?php
	...
	//GET ORDER
	$res=q("SELECT * FROM shop_orders_test WHERE id_order = ".$_POST["order_id"].";", $dbshop, __FILE__, __LINE__);
	...
	if ($_POST["event"]=="Payment")
	{
		...
		if (mysqli_num_rows($res_event_payments)==0)
		{
			...
			q("INSERT INTO shop_orders_events (order_id, event, message, firstmod, firstmod_user, lastmod, lastmod_user) VALUES (".$_POST["order_id"].", 'Payment', '".mysqli_real_escape_string($dbshop, $message)."', ".time().", ".$_SESSION["id_user"].", ".time().", ".$_SESSION["id_user"].");", $dbshop, __FILE__, __LINE__);
		...
		else
		{
			...
			//GET PAYMENT EVENT
			while ($row_event_payments=mysqli_fetch_array($res_event_payments))
			{
			...
			q("UPDATE shop_orders_events SET message = '".mysqli_real_escape_string($dbshop, $message)."', lastmod = ".time().", lastmod_user = ".$_SESSION["id_user"]." WHERE id_event = ".$row_event_payments["id_event"].";", $dbshop, __FILE__, __LINE__);
...
//****************************************************************************************************************************************************
	...
	{
		//GET SHIPPING TIME
		$res_shipping_date=q("SELECT * FROM cms_files WHERE filename = '".$order["ship_number"]."' AND description = 'DHL-Label';", $dbweb, __FILE__, __LINE__);
		if (mysqli_num_rows($res_shipping_date)>0)
		{
			$row_shipping_date=mysqli_fetch_array($res_shipping_date);
		...
		{
			$row_assigned=mysqli_fetch_array($res_assigned);
		...
		if (mysqli_num_rows($res_event_shipment)>0)
		{
			$row_event_shipment=mysqli_fetch_array($res_event_shipment);
			//UPDATE EVENT
q("UPDATE shop_orders_events SET message = '".mysqli_real_escape_string($dbshop, $message)."', lastmod = ".time().", lastmod_user = ".$_SESSION["id_user"]." WHERE id_event = ".$row_event_shipment["id_event"].";", $dbshop, __FILE__, __LINE__);
		}
		else
		{
			//ADD EVENT
			q("INSERT INTO shop_orders_events (order_id, event, message, firstmod, firstmod_user, lastmod, lastmod_user) VALUES (".$_POST["order_id"].", 'Shipment', '".mysqli_real_escape_string($dbshop, $message)."', ".time().", ".$_SESSION["id_user"].", ".time().", ".$_SESSION["id_user"].");", $dbshop, __FILE__, __LINE__);
		}
	}
