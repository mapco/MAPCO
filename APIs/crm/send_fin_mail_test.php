...
//include_once("../../mapco_shop_de/config.php");
	...
	//GET ORDERdata
	$res_order=q("SELECT * FROM shop_orders2 WHERE id_order = ".$_POST["OrderID"].";", $dbshop, __FILE__, __LINE__);
	...
	$response=post(PATH."soa/index.php", array("API" => "crm", "Action" => "crm_get_order_detail_test", "OrderID" => $_POST["OrderID"]) );
	...
	//CREATE MAIL
		...
		{
			$item[$i]=$xml->Order[0]->OrderItems[0]->Item[$i]->OrderItemAmount[0]."x ".$xml->Order[0]->OrderItems[0]->Item[$i]->OrderItemDesc[0];
		}
		...
		SendMail("developer@mapco.de", $sendermail, $subject, $text);
	...
	{
		q("UPDATE shop_orders2 SET fz_fin_mail_count = 1, fz_fin_mail_lastsent = ".time()." WHERE id_order = ".$_POST["OrderID"].";", $dbshop, __FILE__, __LINE__);
	}
