<?php
	...
	function get_order_note_from_conversation($ord_id)
	{
		...
		$res_convers = q("SELECT con.id, con.article_id, art.article, art.lastmod_user FROM crm_conversations AS con, cms_articles AS art WHERE con.order_id=".$ord_id." AND con.type_id=4 AND art.id_article=con.article_id ORDER BY con.firstmod DESC;", $dbweb, __FILE__, __LINE__);
		if ( mysqli_num_rows($res_convers) > 0 )
		{
			$x = 0;
			while($row_convers = (mysqli_fetch_assoc($res_convers)))
			{	
				$art[$x] = '<b>Verkäufernotiz';
				if ( $row_convers['lastmod_user'] > 0 )
				{
					$res_username = q("SELECT username FROM cms_users WHERE id_user=".$row_convers['lastmod_user'], $dbweb, __FILE__, __LINE__);
					$row_username = mysqli_fetch_assoc($res_username);
					$art[$x] .= ' von '.$row_username['username'];
				}
				$art[$x] .= ':</b> ' . $row_convers['article'];
				$x++;
			}
		}
		else
	...
	elseif($_POST["FILTER_SearchFor"]==10 && $_POST["FILTER_Searchfield"]!="")
	{
		...
		if ($invoice_nr!="")
		{
			...
			while ($row=mysqli_fetch_assoc($res))
			{
				// FIX -> show only Mother of combined
				if (($row["combined_with"]>0 && $row["combined_with"] == $row["id_order"]) || $row["combined_with"] < 1 )
				{
					$tmp_orders[]=$row;
				}
				//$tmp_orders[$z]['order_note'] = get_order_note_from_conversation($row['id_order']);
	...
	else
	{
		...
		{
			$filter.=" AND bill_country_code IN ('DE', 'AT', '')";
		}
		if ($_POST["FILTER_Country"]=="international")
		{
			//$filter.=" AND NOT bill_country_code 'DE' AND NOT bill_country_code = 'AT' AND NOT bill_country_code = 'ES' AND NOT bill_country_code = 'PT' AND NOT bill_country_code = 'IT'";
			$filter.=" AND NOT bill_country_code IN ('DE', 'AT', 'ES', 'PT', 'IT', 'AD', 'GQ', 'AR', 'BO', 'CL', 'CR', 'DO', 'EC', 'SV', 'GT', 'HN', 'CO', 'CU', 'MX', 'NI', 'PA', 'PY', 'PE', 'UY', 'VE', 'BR')";
		}
		if ($_POST["FILTER_Country"]=="ES+PT")
		{
			$filter.=" AND bill_country_code IN ('ES', 'PT', 'AD', 'GQ', 'AR', 'BO', 'CL', 'CR', 'DO', 'EC', 'SV', 'GT', 'HN', 'CO', 'CU', 'MX', 'NI', 'PA', 'PY', 'PE', 'UY', 'VE', 'BR')";
		}
		if ($_POST["FILTER_Country"]=="IT")
		{
			$filter.=" AND (bill_country_code = 'IT')";
		}
		...
		if ($_POST["FILTER_SearchFor"]>0 && $_POST["FILTER_Searchfield"]!="")
		{
			...
			switch ($_POST["FILTER_SearchFor"])
			{
				...
				case 13: $searchForCustomerOrderID=true; break;
			}
		}
	...
	//searchForTrackingID
		...
		if ( $searchForCustomerOrderID )
		{
			$qry_string=strtolower(trim($_POST["FILTER_Searchfield"]));
			$res=q("SELECT * FROM shop_orders WHERE NOT ordernr = '' AND ".$rangestart.$rangeend.$filter.$orderby.$limit.";", $dbshop, __FILE__, __LINE__);
			$k=0;
			while ($row=mysqli_fetch_assoc($res))
			{
				if ($row["combined_with"]==0 || $row["combined_with"]==-1 || $row["combined_with"]==$row["id_order"])
				{
					...
					if (strpos(strtolower($row["ordernr"]), $qry_string) !== false )
					{
						$orders[$k]=$row;
						$orders[$k]["entry_pos"]=$k+1;
	...
	foreach ($items as $item)
		{
			...
			$xmldata.="			<OrderItemExchangeRate>".$item["exchange_rate_to_EUR"]."</OrderItemExchangeRate>\n";
