<?php
	...
	if (mysqli_num_rows($res_ebay_items)==0)
	{
		echo '<send_fin_eBayMessageResponse>'."\n";
		echo '	<Ack>Failure</Ack>'."\n";
		echo '	<Error>'."\n";
	...
	$row_ebay_items=mysqli_fetch_array($res_ebay_items);
	...
	if ($_POST["mode"]=="first")
	{
		...
		$message.='Hinweis: Sofern Sie uns bereits Ihre Fahrzeugdaten mitgeteilt haben, brauchen Sie auf diese Nachricht nicht zu antworten.'.chr(13).chr(10).chr(13).chr(10);
	...
	if ($_POST["mode"]=="second")
	{
		...
		$message.='Wir möchten Sie daran erinnern uns die Schlüsselnummern zu 2.1 und 2.2 und DAS DATUM DER ERSTZULASSUNG sowie die komplette Fahrgestellnummer (17-stellig) Ihres Fahrzeugs zu übermitteln, sodass wir Ihnen die passenden Teile zusenden können.'.chr(13).chr(10);
		$message.='Sollten Sie zu Ihrem Fahrzeug keine Schlüsselnummern haben, senden Sie uns stattdessen die allgemeinen Fahrzeugdaten (Hersteller, Modell, Motorisierung, Sonderausstattungen, etc.)'.chr(13).chr(10).chr(13).chr(10);
		$message.='Vielen Dank!';	
		
		$subject='Erinnerung: Bitte teilen Sie uns die Fahrzeugdaten zu Ihrer Bestellung mit';
	}
	...
	if ($_POST["mode"]=="third")
	{
		...
		$message.='Leider haben wir von Ihnen keine Informationen über das/die Fahrzeug(e) erhalten, für welche(s) Sie den/die Artikel bestellt haben. Daher konnten wir vorab nicht prüfen, ob Ihre Bestellung passend ist.'.chr(13).chr(10).chr(13).chr(10);
	
		$message.='Den Versand Ihrer Bestellung werden wir in Kürze vornehmen.'.chr(13).chr(10).chr(13).chr(10);
		$message.='Mit freundlichen Grüßen!';	
		
		$subject='Ihre Bestellung wird in Kürze versandt';
	}
