<?php
	...
	$res=q("SELECT id_list FROM shop_lists WHERE id_list = ".$_POST["itemList"].";", $dbshop, __LINE__, __FILE__);
	...
	$res=q("SELECT item_id FROM shop_lists_items WHERE list_id = ".$_POST["itemList"].";", $dbshop, __LINE__, __FILE__);
	if (mysqli_num_rows($res)==0)
	{
		echo '<send_item_mailResponse>'."\n";
		echo '	<Ack>Failure</Ack>'."\n";
		...
		while ($row=mysqli_fetch_assoc($res))
		{
			...
			{
				$row_shop_item=mysqli_fetch_assoc($res_shop_item);
			}
			...
			if (mysqli_num_rows($res_pic_info)>0) {
				$row_pic_info=mysqli_fetch_assoc($res_pic_info);
				// PATH FOR PIC
...
/*
			...
			*/
			//get vehicles
			$j=0;
			$results=q("SELECT * FROM t_400 WHERE ArtNr='".$row_item_info["MPN"]."' AND (KritNr=2 OR KritNr=16);", $dbshop, __FILE__, __LINE__);
