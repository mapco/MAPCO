...
//*****************************************************************
	...
	if ($_POST["mode"]=="update")
	{
		...
		if (mysql_num_rows($res_check)>0)
		{
			...
			$row_check=mysql_fetch_array($res_check);
			if ($row_check["bill_city"]=="") $update=true; else $update=false;
		}
		else
		{
			$update_OrderID="0";
		...
		//UPDATE DER DATEN WENN ZAHLMETHODE NICHT PAYPAL
		if ($payments_type_id!=4 || $update)
		{

				//UPDATE ORDER
			q("UPDATE shop_orders SET foreign_OrderID = '".mysql_real_escape_string($ebay_orders["OrderID"],$dbshop)."', usermail = '".mysql_real_escape_string($usermail,$dbshop)."', userphone = '".mysql_real_escape_string($ebay_orders["ShippingAddressPhone"],$dbshop)."', bill_firstname = '".mysql_real_escape_string($bill_firstname,$dbshop)."', bill_lastname = '".mysql_real_escape_string($bill_lastname, $dbshop)."', bill_zip = '".mysql_real_escape_string($ebay_orders["ShippingAddressPostalCode"], $dbshop)."', bill_city = '".mysql_real_escape_string($ebay_orders["ShippingAddressCityName"], $dbshop)."', bill_street = '".mysql_real_escape_string($bill_street1, $dbshop)."', bill_number = '".mysql_real_escape_string($bill_streetNumber, $dbshop)."', bill_additional = '".mysql_real_escape_string($ebay_orders["ShippingAddressStreet2"], $dbshop)."', bill_country = '".mysql_real_escape_string($ebay_orders["ShippingAddressCountryName"], $dbshop)."', bill_country_code = '".mysql_real_escape_string($ebay_orders["ShippingAddressCountry"], $dbshop)."', shipping_costs = ".$ebay_orders["ShippingServiceSelectedShippingServiceCost"].", shipping_type_id =".$shippingType.", shipping_details = '".mysql_real_escape_string($shipping_details, $dbshop)."', Payments_TransactionStateDate = ".$paymentdate.", Payments_Type = '".mysql_real_escape_string($ebay_orders["CheckoutStatusPaymentMethod"], $dbshop)."', payments_type_id = ".$payments_type_id.", lastmod = ".time().", lastmod_user = ".$_SESSION["id_user"]."  WHERE foreign_OrderID = '".$update_OrderID."' ;",$dbshop, __FILE__, __LINE__);
...
//UPDATE DATA FOR PAYPALTRANSACTIONS
		{
				//UPDATE ORDER
			q("UPDATE shop_orders SET foreign_OrderID = '".mysql_real_escape_string($ebay_orders["OrderID"],$dbshop)."', shipping_costs = ".$ebay_orders["ShippingServiceSelectedShippingServiceCost"].", shipping_type_id =".$shippingType.", shipping_details = '".mysql_real_escape_string($shipping_details, $dbshop)."', Payments_TransactionStateDate = ".$paymentdate.", Payments_Type = '".mysql_real_escape_string($ebay_orders["CheckoutStatusPaymentMethod"], $dbshop)."', payments_type_id = ".$payments_type_id.", lastmod = ".time().", lastmod_user = ".$_SESSION["id_user"]."  WHERE foreign_OrderID = '".$update_OrderID."' ;",$dbshop, __FILE__, __LINE__);
