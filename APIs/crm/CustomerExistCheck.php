<?php
	...
	*/
	...
	if ( ( $_POST[ 'user_id' ] == 0 or $_POST[ 'user_id' ] == '' ) and ( $_POST[ 'user_mail' ] == '' or $_POST[ 'user_mail' ] == 'Invalid Request' ) ) {
		show_error( 9848, 11, __FILE__, __LINE__, 'User-ID: ' . $_POST[ 'user_id' ] . ', email: ' . $_POST[ 'user_mail' ] );
		exit;
	}
	
	$insert = false;
	
	$select_crm_customer = "SELECT id_crm_customer FROM crm_customers";
	
	$res = q( $select_crm_customer." WHERE user_id=" . $_POST[ 'user_id' ], $dbweb, __FILE__, __LINE__ );
	if ( mysqli_num_rows( $res ) == 0 )
	{
		$data = array();
		$data[ 'user_id' ] = $_POST[ 'user_id' ];
		if ( $_POST[ 'user_mail' ] != '' and $_POST[ 'user_mail' ] != 'Invalid Request' ) 
		{
			$where = "WHERE mail='" . $_POST[ 'user_mail' ] . "'";
