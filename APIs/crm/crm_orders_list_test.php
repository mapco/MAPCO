<?php
	...
	if (isset($_POST["mode"]) && $_POST["mode"]=="single")
	{
		...
		{
			$res=q("SELECT * FROM shop_orders WHERE id_order = ".$_POST["order_id"]." AND shop_id IN (".implode(", ", $usershops).")", $dbshop, __FILE__, __LINE__);
		}
		else
		{
	...
	//FILTER 2 - search for Username
		if ($searchForUserName)
		{
			...
			$k=0;
			while ($row_UserName=mysqli_fetch_array($res_UserName))
			{
	...
	//FILTER 5
		...
		if ($searchForMPN)
		{
			...
			{
				$rowMPN=mysqli_fetch_array($resMPN);
	...
	//FILTER 6
		if ($searchForEbayItemID)
		{
			...
			while ($row_ItemID=mysqli_fetch_array($res_ItemID))
			{
...
//ERSTELLE XML ------------------------------------------------------------------------------------------------
	...
	//GET RETURNS
		...
		if (sizeof($orderids)>0)
		{
			...
			while ($row_returns = mysqli_fetch_array($res_returns))
			{
				if ($row_returns["return_type"] == "return")
				{
					if (isset($returns[$row_returns["order_id"]]))
					{
						$count = sizeof($returns[$row_returns["order_id"]]);
					}
					else
					{
						$count = 0;
					...
					$returns[$row_returns["order_id"]][$count]["return_id"] = $row_returns["id_return"];
					$returns[$row_returns["order_id"]][$count]["firstmod"] = $row_returns["firstmod"];
					$returns[$row_returns["order_id"]][$count]["state"] = $row_returns["state"];
				}
				elseif ($row_returns["return_type"] == "exchange")
				{
					if (isset($exchanges[$row_returns["order_id"]]))
					{
						$count = sizeof($exchanges[$row_returns["order_id"]]);
					}
					else
					{
						$count = 0;
					}
					$exchanges[$row_returns["order_id"]][$count]["return_id"] = $row_returns["id_return"];
					$exchanges[$row_returns["order_id"]][$count]["return_type"] = $row_returns["return_type"];
					$exchanges[$row_returns["order_id"]][$count]["exchange_order_id"] = $row_returns["exchange_order_id"];
					$exchanges[$row_returns["order_id"]][$count]["firstmod"] = $row_returns["firstmod"];
					$exchanges[$row_returns["order_id"]][$count]["state"] = $row_returns["state"];
				}
			}
		}
	...
	for ($k=0; $k<sizeof($orders); $k++)
	{
		...
		{
			$xmldata.=" <returns>\n";
			foreach ($returns[$orders[$k]["id_order"]] as $key => $return)
			{		
				$xmldata.="		<return>\n";
				$xmldata.="			<return_id>".$return["return_id"]."</return_id>\n";
				$xmldata.="			<firstmod>".$return["firstmod"]."</firstmod>\n";
				$xmldata.="			<state>".$return["state"]."</state>\n";
				$xmldata.="		</return>\n";
			}
			$xmldata.="	</returns>\n";
		}
		//Exchanges
		if (sizeof ($exchanges[$orders[$k]["id_order"]])>0)
		{
			$xmldata.=" <exchanges>\n";
			foreach ($exchanges[$orders[$k]["id_order"]] as $key => $exchange)
			{		
				$xmldata.="		<exchange>\n";
				$xmldata.="			<return_id>".$exchange["return_id"]."</return_id>\n";
				$xmldata.="			<exchange_order_id>".$exchange["exchange_order_id"]."</exchange_order_id>\n";
				$xmldata.="			<firstmod>".$exchange["firstmod"]."</firstmod>\n";
				$xmldata.="			<state>".$exchange["state"]."</state>\n";
				$xmldata.="		</exchange>\n";
			}
			$xmldata.="	</exchanges>\n";
		}
	...
	foreach ($items as $item)
		{
			...
			$orderItemTotal=$item["price"]*$item["amount"];
			...
			$xmldata.="			<OrderItemTotal>".number_format(($orderItemTotal*$exchange_rate), 2,",",".")."</OrderItemTotal>\n";
		...
		$xmldata.="	</OrderItems>\n";
		$xmldata.="	<OrderShippingCosts>".number_format(($shipping_costs*$exchange_rate), 2,",",".")."</OrderShippingCosts>\n";
		$xmldata.="	<OrderItemsTotal>".number_format(($orderItemsTotal*$exchange_rate), 2,",",".")."</OrderItemsTotal>\n";
		$xmldata.="	<OrderTotal>".number_format((($orderItemsTotal+$shipping_costs)*$exchange_rate), 2,",",".")."</OrderTotal>\n";
