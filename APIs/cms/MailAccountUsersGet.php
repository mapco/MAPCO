...
*/
	...
	$result = q("SELECT id_user, username, name, firstname, lastname FROM cms_users WHERE (userrole_id=4 OR userrole_id=1) AND active=1;", $dbweb, __FILE__, __LINE__);
	while ( $row = mysqli_fetch_assoc($result) )
	{
		$users[$row['id_user']]['checked'] = 0;
		if ( $row['name'] != '' )
		{
			$users[$row['id_user']]['name'] = $row['name'];
		}
		elseif ( $row['firstname'] != '' || $row['lastname'] != '' )
		{
			$users[$row['id_user']]['name'] = $row['firstname']. ' ' . $row['lastname'];
		}
		else
		{
			$users[$row['id_user']]['name'] = $row['username'];	
		}
	}
