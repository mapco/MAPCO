...
*/
	...
	$result = q("SELECT user FROM cms_mail_accounts WHERE id_account=".$_POST['account_id'].";", $dbweb, __FILE__, __LINE__);
	$row = mysqli_fetch_assoc($result);
	
	$result = q("SELECT id_account FROM cms_mail_accounts WHERE user='".$row['user']."';", $dbweb, __FILE__, __LINE__);
	while ( $row = mysqli_fetch_assoc($result) )
	{
		if ( $_POST['user_ids'] != '' )
		{
			$data = array();
			$data['account_id'] = $row['id_account'];
			$data['firstmod'] = time();
			$data['firstmod_user'] = $_SESSION['id_user'];
			$data['lastmod'] = $data['firstmod'];
			$data['lastmod_user'] = $_SESSION['id_user'];
			...
			{
				$cms_mail_accounts_users = implode(',',$cms_mail_accounts_users);
				q("DELETE FROM cms_mail_accounts_users WHERE account_id=".$row['id_account']." AND user_id IN (".$cms_mail_accounts_users.");", $dbweb, __FILE__, __LINE__);
			}
		}
		else
		{
			q("DELETE FROM cms_mail_accounts_users WHERE account_id=".$row['id_account'].";", $dbweb, __FILE__, __LINE__);	
		}
	}
?>