...
?>
	<script>
		...
		function view_cart()
		{
			$.post("<?php echo PATH; ?>modules/shop_cart_actions.php", { action:"view", lang:"<?php echo $_SESSION["lang"]; ?>", id_user:<?php echo $_SESSION["id_user"]; ?> },
				function(data)
				{
					$("#view").html(data);
					waypoints_reload();
...
<?php
	...
	if (isset($_GET["PayPalAction"]) && $_GET["PayPalAction"]=="abort") {
		...
		echo '<script>payment_select();</script>';
	}
	...
	if ($paypalpayment && $paypalpaymentOK && !$paypalCheckout) {
		...
		echo '<script>GetPayPalCheckout();</script>'; 
		exit;
	}
	...
	if (isset($_POST["form_button"]) || $paypalCheckout and !isset($_POST["cartupdate"]) and !isset($_POST["cartclear"]))
	{
		...
		else
		{
			...
			if (isset($_SESSION["id_payment"]) && in_array($_SESSION["id_payment"], $paypal_IDs) && !$paypalpaymentOK && !$paypalCheckout) {
				echo '<div class="success">'.t("Sie werden auf die PayPal-Seite weitergeleitet").'....!!!!</div><br />';
				echo '<a href="javascript:SetPayPalCheckout('.$_SESSION["id_user"].');">Manuelle Weiterleitung</a>';
				//echo '<script>';
				//SetPayPalCheckout('.$_SESSION["id_user"].');
                //echo '<script>'; 
				exit;}
			...
			if($frachtpauschale) $_SESSION["shipping_details"]=$_SESSION["shipping_details"].' (Frachtpauschale)';
			...
			//Warenkorb leeren
			echo '<script language="javascript"> cart_clear2(); </script>';
	...
	{
		$results=q("SELECT * FROM shop_carts AS a, shop_items AS b, shop_items_".$_SESSION["lang"]." AS c WHERE a.user_id='".$_SESSION["id_user"]."' AND item_id=b.id_item AND b.id_item=c.id_item;", $dbshop, __FILE__, __LINE__);
		while($row=mysql_fetch_array($results))
		{
