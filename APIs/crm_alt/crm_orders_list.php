<?php
	...
	if ($_POST["OrderBy"]!="" && $_POST["OrderDirection"]=="+") $orderby.=" DESC";
...
//FILTER 0
	if ($searchForAll)
	{
		...
		$res=q("SELECT shop_id, foreign_OrderID, crm_customer_id, id_order, usermail, bill_company, bill_firstname, bill_lastname, shipping_costs, firstmod, fz_fin_mail_count, fz_fin_mail_lastsent, shipping_details FROM shop_orders WHERE ".$rangestart.$rangeend.$filter.$orderby.$limit.";", $dbshop, __FILE__, __LINE__);
		$k=0;
		...
		if (($_POST["FILTER_Platform"]==3 || $_POST["FILTER_Platform"]==4) && !$qry_string=="" )
		{
			...
			while ($row_UserName=mysql_fetch_array($res_UserName))
			{
				...
				if (strpos(strtolower(" ".$row_UserName["BuyerUserID"]), $qry_string)>0 )
				{
					...
					$res=q("SELECT shop_id, foreign_OrderID, crm_customer_id, id_order, usermail, bill_company, bill_firstname, bill_lastname, shipping_costs, firstmod, fz_fin_mail_count, fz_fin_mail_lastsent, shipping_details FROM shop_orders WHERE foreign_OrderID = ".$row_UserName["OrderID"]." AND ".$rangestart.$rangeend.$filter.$orderby.$limit.";", $dbshop, __FILE__, __LINE__);
...
//FILTER 3 & 4
	if ($searchForName || $searchForAdresse)	
	{
		...
		$res=q("SELECT shop_id, foreign_OrderID, crm_customer_id, id_order, usermail, bill_company, bill_firstname, bill_lastname, shipping_costs, bill_zip, bill_city, bill_street, bill_number, bill_additional, bill_country, firstmod, fz_fin_mail_count, fz_fin_mail_lastsent, shipping_details FROM shop_orders WHERE ".$rangestart.$rangeend.$filter.$orderby.";", $dbshop, __FILE__, __LINE__);
		while ($row=mysql_fetch_array($res))
		{
			$match=true;
...
//FILTER 5
	...
	if ($searchForMPN)
	{
		...
		if (mysql_num_rows($resMPN)>0) 
		{
			...
			$res=q("SELECT shop_id, foreign_OrderID, crm_customer_id, id_order, usermail, bill_company, bill_firstname, bill_lastname, shipping_costs, firstmod, fz_fin_mail_count, fz_fin_mail_lastsent, shipping_details FROM shop_orders WHERE ".$rangestart.$rangeend.$filter.$orderby.";", $dbshop, __FILE__, __LINE__);
...
//FILTER 6
	if ($searchForEbayItemID)
	{
		...
		while ($row_ItemID=mysql_fetch_array($res_ItemID))
		{
			...
			$res=q("SELECT shop_id, foreign_OrderID, crm_customer_id, id_order, usermail, bill_company, bill_firstname, bill_lastname, shipping_costs, firstmod, fz_fin_mail_count, fz_fin_mail_lastsent, shipping_details FROM shop_orders WHERE foreign_OrderID = ".$row_ItemID["OrderID"]." AND ".$rangestart.$rangeend.$filter.$orderby.$limit.";", $dbshop, __FILE__, __LINE__);
		...
		{
			$res=q("SELECT shop_id, foreign_order_id, crm_customer_id, id_order, usermail, bill_company, bill_firstname, bill_lastname, shipping_costs, firstmod, fz_fin_mail_count, fz_fin_mail_lastsent FROM shop_orders WHERE ".$rangestart.$rangeend.$filter.$orderby.$limit.";", $dbshop, __FILE__, __LINE__);
			$k=0;
			while ($row=mysql_fetch_array($res))
			{
...
//ERSTELLE XML ------------------------------------------------------------------------------------------------
	for ($k=0; $k<sizeof($orders); $k++)
	{
		...
		$xmldata.=" <orderDate>".date("d.m.Y H:i", $orders[$k]["firstmod"])."</orderDate>\n";
