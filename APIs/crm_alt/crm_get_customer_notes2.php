<?php
	...
	if ( !isset($_POST["mode"]) )
	{
		...
		echo '		<shortMsg>Ausgabe-Modus nicht gefunden</shortMsg>'."\n";
		echo '		<longMsg>Es muss angegeben werden, die zur Liste hinzugefügt werden soll.</longMsg>'."\n";
		echo '	</Error>'."\n";
		echo '</crm_get_customer_notesResponse>'."\n";
		exit;
	}
	...
	if ( !isset($_POST["customer_id"]) )
	{
		...
		echo '		<longMsg>Es muss eine Kunden ID angegeben werden, zu der Notizen angezeigt werden sollen.</longMsg>'."\n";
		echo '	</Error>'."\n";
		echo '</crm_get_customer_notesResponse>'."\n";
		exit;
	}
	
	if ($_POST["mode"]=="order" && !isset($_POST["order_id"]))
	{
		echo '<crm_get_customer_notesResponse>'."\n";
		echo '	<Ack>Failure</Ack>'."\n";
		echo '	<Error>'."\n";
		echo '		<Code>'.__LINE__.'</Code>'."\n";
		echo '		<shortMsg>Order ID nicht gefunden.</shortMsg>'."\n";
		echo '		<longMsg>Es muss eine Order ID angegeben werden, wenn der Ausgabemodus `Order` ist.</longMsg>'."\n";
		echo '	</Error>'."\n";
		echo '</crm_get_customer_notesResponse>'."\n";
		exit;
	}
	...
	if ($_POST["mode"]=="order") $res_notes=q("SELECT * FROM crm_customer_notes2 WHERE customer_id = ".$_POST["customer_id"]." AND order_id = ".$_POST["order_id"].";", $dbweb, __FILE__, __LINE__);
	if ($_POST["mode"]=="all") $res_notes=q("SELECT * FROM crm_customer_notes2 WHERE customer_id = ".$_POST["customer_id"].";", $dbweb, __FILE__, __LINE__);
	if ($_POST["mode"]=="customer") $res_notes=q("SELECT * FROM crm_customer_notes2 WHERE customer_id = ".$_POST["customer_id"]." AND order_id = 0;", $dbweb, __FILE__, __LINE__);
	
	
	$notes_count=mysql_num_rows($res_notes);
	if ($notes_count>0)
	{
		...
		while ($row_contacts=mysql_fetch_array($res_contacts))
		{
