<?php
	...
	if (mysql_num_rows($res)==0)
	{
		...
		while ($row=mysql_fetch_array($res))
		{
			...
			if (mysql_num_rows($res_shop_item)==0) $warning[$row["item_id"]];
			else
			{
				$row_shop_item=mysql_fetch_array($res_shop_item);
			}
			...
			if (mysql_num_rows($res_pic_info)>0) {
				$row_pic_info=mysql_fetch_array($res_pic_info);
				// PATH FOR PIC
			...
			if (mysql_num_rows($res_item_info)==0) $warning[$row["item_id"]];
			else
			{
				$row_item_info=mysql_fetch_array($res_item_info);
			}

			//ARTNR beziehen
			$res_item_info2=q("SELECT * FROM shop_items WHERE id_item = ".$row["item_id"].";", $dbshop, __FILE__, __LINE__);
			if (mysql_num_rows($res_item_info2)==0) $warning[$row["item_id"]];
			else
			{
				$row_item_info2=mysql_fetch_array($res_item_info2);
			}
