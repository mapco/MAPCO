...
//require_once '../payments/PayPalConstants.php';
echo "AUFGERUFEN!!!!!!";
...
/*
	...
	*/
	...
	$res=q("insert into payment_notification_messages3 (message, date_recieved) values ('".mysql_real_escape_string($text, $dbshop)."', ".time().");", $dbshop, __FILE__, __LINE__);
	echo "#".$pnm_id=mysql_insert_id($dbshop);
	...
	if ($payment_status=="Completed")
	{
		...
		if (isset($for_auction) && ($for_auction=="true" || $for_auction=="TRUE"))
		{
			...
			//if (mysql_affected_rows()==1)
			{
				...
				if (mysql_num_rows($res_ebay_order)==1)
				{
...
echo mysql_num_rows($res_parent);
...
//**************************************************************************
	...
	if ($processed)
	{
		$sql ="INSERT INTO payment_notifications3 (".
...
if (isset($Userdata))
{
	...
	$res=q("insert into payment_notification_messages3 (message, date_recieved, processed) values ('".mysql_real_escape_string($nvp, $dbshop)."', ".time().", 0);", $dbshop, __FILE__, __LINE__);
		$pnm_id=mysql_insert_id($dbshop);
	...
	//TOTAL
		...
		$res=q("INSERT INTO payment_notifications3 (payment_account_id, PN_date, payment_date, state, state_reason, orderTransactionID,shop_orderID,total, fee, platform, buyer_id, payment_type, buyer_lastname, buyer_firstname, buyer_mail, paymentTransactionID) VALUES (".$ACCOUNTID.", ".time().", ".$Payments_TransactionStateDate.", '".mysql_real_escape_string($Status, $dbshop)."', '".mysql_real_escape_string($Description, $dbshop)."', '".mysql_real_escape_string($TransID, $dbshop)."', '".mysql_real_escape_string($id_order, $dbshop)."', '".$TOTAL."', '0', '".$PLATFORM."', '".$buyer_id."', '".$PAYMENTTYPE."','".$buyer_lastname."', '".$buyer_firstname."', '".$buyer_mail."', '".mysql_real_escape_string($PayID, $dbshop)."');", $dbshop, __FILE__, __LINE__);
