...
echo "EBAYORDERID: ".$_POST["EbayOrderID"]."<br />";
	$res_orders=q("SELECT * FROM ebay_orders3 WHERE id_order = ".$_POST["EbayOrderID"].";", $dbshop, __FILE__, __LINE__);
	...
	$res_orders_items=q("SELECT * FROM ebay_orders_items3 WHERE OrderID = '".$ebay_orders["OrderID"]."';", $dbshop, __FILE__, __LINE__);
...
//INSERT SHOP ORDER ++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	//			echo "UPDATE IPN ".$IPNs[$i]["id_PN"]."<br />";
		...
		for ($i=0; $i<sizeof ($ebay_orders_items); $i++)
		{
			...
			q("INSERT INTO shop_orders_items_crm2 (order_id, foreign_transactionID, item_id, amount, price, netto) VALUES (".$order_id.", '".$ebay_orders_items[$i]["TransactionID"]."', ".$row_item["id_item"].", ".$ebay_orders_items[$i]["QuantityPurchased"].", ".$ebay_orders_items[$i]["TransactionPrice"].", 0);", $dbshop, __FILE__, __LINE__);
			
			echo "ADD ADD ITEM ".$order_id." - TR: ".$ebay_orders_items[$i]["TransactionID"]."<br />";
		}
...
//*****************************************************************
	...
	if ($_POST["mode"]=="update")
	{
		...
		for ($i=0; $i<sizeof($ebay_orders_items); $i++)
		{
			$res_check_items=q("SELECT * FROM shop_orders_items_crm2 WHERE foreign_TransactionID = '".$ebay_orders_items[$i]["TransactionID"]."' ;", $dbshop, __FILE__, __LINE__);
			if (mysql_num_rows($res_check_items)>0)
			{
				...
				$res_check_order=q("SELECT * FROM shop_orders_crm2 WHERE id_order = ".$row_check_items["order_id"].";",$dbshop, __FILE__, __LINE__);
		...
		//FESTLEGEN DER ZU UPDATENDEN ORDER
			//Search for OrderID
		$res_check=q("SELECT * FROM shop_orders_crm2 WHERE foreign_OrderID = '".$ebay_orders["OrderID"]."' ;", $dbshop, __FILE__, __LINE__);
		if (mysql_num_rows($res_check)>0)
		{
		...
		for ($i=0; $i<sizeof($old_transactions); $i++)
		{
			...
			{
				q("DELETE FROM shop_orders_crm2 WHERE foreign_OrderID ='".$old_transactions[$i]["OrderID"]."';", $dbshop, __FILE__, __LINE__);
				echo "SHOP DELETE OLD ORDER: ".$old_transactions[$i]["OrderID"]."<br />";
			}
		}
		...
		for ($i=0; $i<sizeof($ebay_orders_items); $i++)
		{
			...
			$res_check=q("SELECT * FROM shop_orders_items_crm2 WHERE foreign_TransactionID = '".$ebay_orders_items[$i]["TransactionID"]."' ;", $dbshop, __FILE__, __LINE__);
			if (mysql_num_rows($res_check)>0) 
			{
				//UPDATE TRANSACTION
				$res_item=q("SELECT id_item FROM shop_items WHERE MPN = '".$ebay_orders_items[$i]["ItemSKU"]."';", $dbshop, __FILE__, __LINE__);
				...
				q("UPDATE shop_orders_items_crm2 SET order_id = ".$id_order.", item_id = ".$row_item["id_item"].", amount = ".$ebay_orders_items[$i]["QuantityPurchased"].", price = ".$ebay_orders_items[$i]["TransactionPrice"].", netto = 0 WHERE foreign_TransactionID = '".$ebay_orders_items[$i]["TransactionID"]."' ;", $dbshop, __FILE__, __LINE__); 
				echo "UPDATE known ITEM ".$id_order." - TR: ".$ebay_orders_items[$i]["TransactionID"]."<br />";
			...
			else
			{
				...
				q("INSERT INTO shop_orders_items_crm2 (order_id, foreign_TransactionID, item_id, amount, price, netto, customer_vehicle_id) VALUES (".$id_order.", '".$ebay_orders_items[$i]["TransactionID"]."', ".$row_item["id_item"].", ".$ebay_orders_items[$i]["QuantityPurchased"].", ".$ebay_orders_items[$i]["TransactionPrice"].", 0, 0);", $dbshop, __FILE__, __LINE__);	
				
				echo "UPDATE ADD ITEM: ".$id_order." - TR: ".$ebay_orders_items[$i]["TransactionID"]."<br />";
			}
		}
		...
		for ($i=0; $i<sizeof($old_transactions); $i++)
		{
			...
			if ($no_match)
			{
				q("UPDATE shop_orders_items_crm2 SET order_id=0 WHERE foreign_TransactionID = '".$old_transactions[$i]["TransactionID"]."';", $dbshop, __FILE__, __LINE__);
				echo "SHOP UNLINK TR :".$old_transactions[$i]["TransactionID"]."<br />";
			}
		}
