...
*******************************************************************************/
	...
	//speichert, ob eine Fehlerausgabe durch show_error(id_errorcode, id_errortype, __FILE__, __LINE__, text , show_error == true) erzeugt wurde
		//Fehler ohne Ausgabe werden nicht berücksichtigt
	$errorXML_shown = false;
	...
	if (isset($_POST["APIRequest"])
		...
		OR $_POST["APIRequest"] == "OrderGet"
		OR $_POST["APIRequest"] == "OrderEventSet"
		OR $_POST["APIRequest"] == "OrderUpdate"
		OR $_POST["APIRequest"] == "OrderItemAdd"
		OR $_POST["APIRequest"] == "PaymentNotificationHandler"
...
*/
	...
	*/
	...
	{
		global $dbweb;
		global $errorXML_shown;

		//TRACKEN, OB EIN FEHLER AUSGEGEBEN WURDE
			//benötigt für Funktion service_exit()
		if ( $show_error ) 
		{
			$errorXML_shown = true;
		}
		...
		if (mysqli_num_rows($results) == 0) {
			...
			$responseXML = post(PATH."soa2/", array( "API" => "cms", "APIRequest" => "ErrorAdd", "id_errortype" => 8, "id_errorcode" => 9796, "file" => $file, "line" => $line, "text" => "ErrorTypeID: ".$id_errortype." ErrorCode: ".$id_errorcode." ".print_r($_POST, true)." FEHLERTEXT: ".$text ));
			...
			$responseXML = post(PATH."soa2/", array( "API" => "cms", "APIRequest" => "ErrorAdd", "id_errortype" => $id_errortype, "id_errorcode" => $id_errorcode, "file" => $file, "line" => $line, "text" => $text ) );
	...
	{
		post(PATH."soa2/", array( "API" => "cms", "APIRequest" => "ErrorAdd", "id_errortype" => $id_errortype, "id_errorcode" => $id_errorcode, "file" => $file, "line" => $line, "text" => $text ) );
	}
	...
	function service_exit ()
	{
		...
		//PRÜFUNG, ob ein Fehler ausgegeben wurde
			// WENN JA -> es existiert bereits eine valide XML Ausgabe durch show_error()
			// WENN NEIN -> XML Ausgabe muss noch vervollständigt werden
		if ( !$errorXML_shown )
		{
			$end = microtime(true);
			$diff = $start - $end;
	...
	function define_xml_namespace ( $definitions )
	{
		$definition_string = '';
		foreach ( $definitions as $name => $definition )
		{
			if ( $definitions_string != '')
			{
				$definitions_string .= ' ';
			}
			$definitions_string .= 'xmlns:'.$name.'="'.$definition.'"';
		}
		...
		echo '<' . $_POST["APIRequest"] . 'Response '.$definitions_string.'>' . "\n";
	...
	function check_response ( $xml )
	{
		...
		{
			show_error(11359, 8, __FILE__, __LINE__, "FEHLERHAFTES XML: ".$xml, true);
			exit;
		}
		libxml_clear_errors();
		libxml_use_internal_errors($use_errors);
	...
	else 
	{
		...
		//start service
		include($file);
		//ab hier wird durch ein exit im Service abgebrochen
			// VALIDES XML wird durch show_error() erzeugt -> vorherige Ausgabe (im Puffer) wird gelöscht und durch das Fehler XML ersetzt
			// VALIDES XML wird durch service_exit() erzeugt -> vorherige Ausgabe (im Puffer) wird zu einem validem XML ergänzt
			$end = microtime(true);
			$diff = $start - $end;
		echo '	<Runtime>' . $diff . '</Runtime>' . "\n";
		echo '</'. $_POST["APIRequest"] . 'Response>' . "\n";
	...
	//VALIDIERUNG DES ZURÜCKZUGEBENDEN XMLs
	if (isset($_POST['ValidateResponse']) && $_POST['ValidateResponse'] == true)
	{
		check_response( ob_get_contents() );
	}
