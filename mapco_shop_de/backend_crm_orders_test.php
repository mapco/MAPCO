...
<script src="javascript/crm/OrderReturns_test.php" type="text/javascript" /></script>
				...
				$(this).children().each(
					function()
					{
						...
						switch ($tagname)
						{
							...
							case "credits": 
								orders[order_id]["credits"] = new Object();
								$(this).find("credit").each(
								function ()
								{
									var credit_id = $(this).find("shop_order_credit_id").text();
									orders[order_id]["credits"][credit_id] = new Object();
									$(this).children().each(
									function ()
									{
										var $tagname2=this.tagName;
										if ($tagname2 == 'shop_order_credit_returnlabels' )
										{
											orders[order_id]["credits"][credit_id]['returnlabel'] = new Object();
											var $labelcounter = 0;
											$(this).find('shop_order_credit_returnlabel').each( function ()
											{
												orders[order_id]["credits"][credit_id]['returnlabel'][$labelcounter] = new Object();
												orders[order_id]["credits"][credit_id]['returnlabel'][$labelcounter]['id'] = $(this).find('shop_order_credit_returnlabelID').text();
												orders[order_id]["credits"][credit_id]['returnlabel'][$labelcounter]['firstmod'] = $(this).find('shop_order_credit_returnlabel_firstmod').text();
										...
										{
											orders[order_id]["credits"][credit_id][$tagname2] = $(this).text();
										}
									});
...
//				alert(data);
	...
	//BESTELLUNG NACH IDIMS NICHT MEHR BEARBEITBAR -> NUR ANZEIGEN
		...
		if ( orders[orderid]["status_id"]!=4 && orders[orderid]["status_id"]!=8 )
		{
			...
			//RÜCKGABE	
			menu+='	<li style="margin:5px; margin-left:-22px;"><a href="javascript:order_returns_add('+orderid+');">Rückgabefall anlegen</a></li>';
			//EXCHANGE
			menu+='	<li style="margin:5px; margin-left:-22px;"><a href="javascript:order_exchange_add('+orderid+');">Umtauschfall anlegen</a></li>';
			//TEILUMTAUSCH
			menu+='	<li style="margin:5px; margin-left:-22px;"><a href="javascript:order_returns_partial_dialog();">Teilumtausch anlegen</a></li>';
			//CREDIT
			menu+='	<li style="margin:5px; margin-left:-22px;"><a href="javascript:order_credit_add('+orderid+');">Gutschrift anlegen</a></li>';
...
*/
		//RETRUNS
		if (typeof (orders[orderid]["credits"])!=="undefined" || typeof (orders[orderid]["credits"])!=="undefined")
		{
			html+='==================<br />';
			
			$c_counter = 0;
			for ( var $credit_id in orders[orderid]['credits'] )
			{
				if ($c_counter > 0)
				{
					html += '<span style="width:100%; text-align:center">---------------------------</span><br />';
				}
				html+='	<div id="order_credit'+orderid+'" style="width:100%; float:left; margin:3px;">';
				if ( orders[orderid]['credits'][$credit_id]['shop_order_credit_type'] == 'exchange' )
				{
					html+='		<span><img style="margin:0px 0px 0px 0px; border:0; padding:0; float:left; cursor:pointer;" onclick="order_credit_start('+$credit_id+');" src="images/crm/Shipment_Exchange.png" alt="Rückgabefall" title="Umtausch" /></span>';	
				}
				if ( orders[orderid]['credits'][$credit_id]['shop_order_credit_type'] == 'return' )
				{
					html+='		<span><img style="margin:0px 0px 0px 0px; border:0; padding:0; float:left; cursor:pointer;" onclick="order_credit_start('+$credit_id+');" src="images/crm/Shipment_Returned.png" alt="Rückgabefall" title="Rückgabe" /></span>';	
				}
				if ( orders[orderid]['credits'][$credit_id]['shop_order_credit_type'] == 'credit' )
				{
					html+='		<span><img style="margin:0px 0px 0px 0px; border:0; padding:0; float:left; cursor:pointer;" onclick="order_credit_start('+$credit_id+');" src="images/crm/Credit.png" alt="Rückgabefall" title="Gutschrift" /></span>';	
				}

				html+='		<span>'+convert_time_from_timestamp(orders[orderid]['credits'][$credit_id]['shop_order_credit_firstmod'], 'complete')+'</span>';
	
				if (orders[orderid]['credits'][$credit_id]['shop_order_credit_status']==0)
				{
					html+='		<span><img style="margin:0px 0px 0px 0px; border:0; padding:0; float:right;" src="images/icons/16x16/warning.png" alt="Fall noch nicht abgeschlossen" title="Fall noch nicht abgeschlossen" /></span>';	
				}
				html+='	</div>';
				//ADD RETURNLABELS
				if ( typeof (orders[orderid]["credits"][$credit_id]["returnlabel"])!=="undefined" )	
				{
					//html+='<br />';
					
					for ( var $index in orders[orderid]["credits"][$credit_id]["returnlabel"] )	
					{
						html += '<div style="width:100%; float:left; margin:3px;">';
						html +='		<img style="margin:0px 0px 0px 0px; border:0; padding:0; float:left;" src="images/crm/Shipment_info.png" alt="Retourlabel" title="Retourlabel" />';				
						html +='		<span><small><a href="http://nolp.dhl.de/nextt-online-public/set_identcodes.do?lang=de&idc='+orders[orderid]["credits"][$credit_id]["returnlabel"][$index]["id"]+'&rfn=&extendedSearch=true" target="_blank">'+orders[orderid]["credits"][$credit_id]["returnlabel"][$index]["id"]+'</a></small><br />';
						html +='		<small>'+convert_time_from_timestamp(orders[orderid]["credits"][$credit_id]["returnlabel"][$index]["firstmod"], 'complete')+'</small></span>';
						html += '</div>';	
					}
	...
	function view_box(orderid, next_action)
	{
		...
		FILTER_Searchfield=$("#FILTER_Searchfield").val();
		wait_dialog_show();
		$.post("<?php echo PATH; ?>soa/", { API: "crm", Action: "crm_orders_list_test", mode:mode, order_id:orderid, FILTER_Platform:FILTER_Platform, FILTER_Status:FILTER_Status, FILTER_Country:FILTER_Country, FILTER_SearchFor:FILTER_SearchFor, FILTER_Searchfield:FILTER_Searchfield, FILTER_Ordertype:FILTER_Ordertype, OrderBy:OrderBy, OrderDirection:OrderDirection, ResultPage:ResultPage,ResultRange:ResultRange, FILTER_DateSearchFor:FILTER_DateSearchFor, date_from:date_from, date_to:date_to},
			function(data)
			{
