					...
					function()
					{
						...
						switch ($tagname)
						{
							...
							case "returns": 
								...
								function ()
								{
									var return_id = $(this).find("return_id").text();
							...
							case "exchanges": 
								orders[order_id]["exchanges"] = new Object();
								$(this).find("exchange").each(
								function ()
								{
									var return_id = $(this).find("return_id").text();
									orders[order_id]["exchanges"][return_id] = new Object();
									$(this).children().each(
									function ()
									{
										var $tagname2=this.tagName;
										orders[order_id]["exchanges"][return_id][$tagname2] = $(this).text();
									});
								});
								break;
...
//				alert(data);
	...
	function IDIMS_Order_check(orderid)
	{
		...
		if (orders[orderid]["ordertype_id"] == 1 || orders[orderid]["ordertype_id"] == 2 || orders[orderid]["ordertype_id"] == 3)
		{
			...
			if (orders[orderid]["status_id"]==4) return false;
	...
	//AUFTRAG GESCHRIEBEN
		if ( (orders[orderid]["PaymentTypeID"]==1 || orders[orderid]["PaymentTypeID"]==7) && orders[orderid]["shipping_type_id"]!=15 && orders[orderid]["AUF_ID"]==0)
		{
	...
	//BESTELLUNG NACH IDIMS NICHT MEHR BEARBEITBAR -> NUR ANZEIGEN
		...
		if (orders[orderid]["AUF_ID"]==0 || <?php echo $_SESSION["userrole_id"]; ?>==1 || user_id == 22733)
		{
	...
	//RETOURLABEL	
		if (DHL_RetourLabelParameter[orders[orderid]["bill_country_code"]] !== undefined)
		{
			var dhl_parameter = DHL_RetourLabelParameter[orders[orderid]["bill_country_code"]]["dhl_parameter"];
	...
	//BESTELLVERLAUF
		menu+='<li style="margin:3px; margin-left:-22px;"><a href="javascript:show_order_events('+orderid+');">Bestellverlauf</a></li>';
	...
	menu+='	<li style="margin:5px; margin-left:-22px;"><a href="javascript:payments_dialog_load('+orderid+');">Zahlung bearbeiten NEU</a></li>';
	}
	...
	//EXCHANGE
		var $hasExchange=0;
		if (typeof (orders[orderid]["exchanges"])!=="undefined")
		{
			$.each(orders[orderid]["exchanges"], function($returnid, $returnfield)
			{
...
*/				
	...
	function user_contact(order_id, con_types_data, $shop_mail)
	{
		...
		var tr = $('<tr></tr>');
			...
			for(b in con_types_data)
			{
				...
				{
					option = $('<option value="'+con_types_data[b]["id"]+'">'+con_types_data[b]["type"]+'</option>');
		...
		table.append(tr);
		tr = $('<tr></tr>');
			td = $('<td style="font-weight: bold;">Empfänger (email-Adr, Tel-Nr...):</td>');
			tr.append(td);
			td = $('<td><input type="text" id="con_receiver" style="width: 400px;"></td>');
			tr.append(td);
		table.append(tr);
		tr = $('<tr></tr>');
	...
	function show_order_buyer(orderid)
	{
		...
		else
		{
			...
			{
				html+='<tr>';
				html+='<td style="border:0;"><small><b>'+orders[orderid]["usermail"]+'</b></small></td>';
	...
	function show_order_state(orderid)
	{
		...
		//ANZEIGE FÜR BESTELLUNGEN
		{
			...
			{
				//PENDING
				if (orders[orderid]["Payments_TransactionState"]=="" || orders[orderid]["Payments_TransactionState"]=="Pending")
				{
			...
			{
				if (orders[orderid]["Payments_TransactionState"]=="Pending" || orders[orderid]["Payments_TransactionState"]=="")
				{
		...
		if (typeof (orders[orderid]["exchanges"])!=="undefined")
		{
			$.each(orders[orderid]["exchanges"], function($returnid, $returnfield)
			{
				$hasExchange = $returnid;
				$firstmod = $returnfield["firstmod"];
				$exchange_orderid = $returnfield["exchange_order_id"];
				$returnstate = $returnfield["state"];
			});
				...
				//	table+='	<th style="width:90px;"><img style="margin:0px 5px 0px 0px; border:0; padding:0; float:right; cursor:pointer;" src="images/icons/24x24/mail_add.png" alt=Fahrzeugdatenanfragen senden" title="Fahrzeugdatenanfragen senden" onclick="send_fin_mail_checked_dialog();" /></th>';
					...
					for (var pos in entry_pos) {
						...
						table+='<tr id="orderdata'+OrderID+'" style="background-color:'+rowcolor+'">';
							...
							table+='		<small>';
							table+='			<br>O:&nbsp;'+OrderID;
							if( orders[OrderID]["VPN"]!=OrderID ) table+='			<br>E:&nbsp;'+orders[OrderID]["VPN"];
							if( orders[OrderID]["AUF_ID"]>0 ) table+='			<br>A:&nbsp;'+orders[OrderID]["AUF_ID"];
							if( orders[OrderID]["invoice_nr"]!="" ) table+='			<br>R:&nbsp;'+orders[OrderID]["invoice_nr"];
							table+='	</small>';
							table+='</td>';
...
//show_status2(data);
	...
	{
		wait_dialog_show();		
		$.post("<?php echo PATH; ?>soa2/", { API: "shop", APIRequest: "OrderDetailGet", OrderID:orderid},
		function(data)
		{
			wait_dialog_hide();
			var $xml=$($.parseXML(data));
...
//	alert(href);
	...
	function order_add()
	{
		...
		html+='	<td>';
		html+='		<select id="order_add_site" size="1">';
					...
					{ text: "Bestellung anlegen", click: function() { 
						...
						else
						{
							...
							*/
							...
							//BEI MANUELL ANGELEGTEN BESTELLUNGEN IST CURRENCY IMMER EUR
							var currency = "EUR";
							
							//SETZE SHOP ID ANHAND DER SITE_ID -> Immer Basisshop
						/*
							for (var id_shop in Shop_Shops) 
							{
								if (Shop_Shops[id_shop]["site_id"] == $("#order_add_site").val() && Shop_Shops[id_shop]["active"]==1)
								{
									var shop_id = id_shop;
								}
							}
	...
	function order_update_set_payment(orderid, payment_type_id)
	{
		...
		$.post("<?php echo PATH; ?>soa2/index.php", { API: "shop", APIRequest: "OrderDetailGet",  OrderID:orderid},
			function($data)
			{
...
*/
	...
	function add_orderpositions(parentorderid, shop_id)
	{
		...
		$.post("<?php echo PATH; ?>soa2/", { API: "shop", APIRequest: "OrderDetailGet", OrderID:parentorderid},
			function(data)
			{
				//alert(data);
				var $xml=$($.parseXML(data));
	...
	//	parentorderid=1749450;
		...
		if (Shop_Shops[shop_id]["shop_type"]==2)
		{
			...
			$.post("<?php echo PATH; ?>soa2/index.php", { API: "shop", APIRequest: "OrderDetailGet",  OrderID:parentorderid},
			function($data)
			{
				try { $xml = $($.parseXML($data));	} catch (err) {	show_status2(err.message); return;}
				var $Ack = $xml.find("Ack").text();
