...
//		echo $row["GART"].' GenArt<br />';
	...
	for ($i=0; $i<$max; $i++)
	{
		q("INSERT INTO cms_articles (title, imageprofile_id, firstmod, lastmod) VALUES('".mysqli_real_escape_string($dbweb, get_title($new[$i], $sprache[$j]))."', 4, ".time().", ".time().");", $dbweb, __FILE__, __LINE__);
		...
		q("INSERT INTO shop_items (Brand, MPN, collateral, article_id, menuitem_id, firstmod, lastmod) VALUES('MAPCO Autotechnik GmbH', '".$new[$i]."', ".$t200_collateral[$new[$i]].", ".$article_id.", '".get_menuitem_id($new[$i], $sprache[$j])."', '".time()."', '".time()."');", $dbshop, __FILE__, __LINE__);
...
//		if ($stoptime-$starttime>60) exit;
	...
	$time=0;
	$results=q("SELECT * FROM shop_items ORDER BY lastmod;", $dbshop, __FILE__, __LINE__);
	while( $row=mysqli_fetch_array($results) )
	{
		$responseXml = post(PATH."soa/", array("API" => "shop", "Action" => "ItemUpdate", "id_item" => $row["id_item"]));
		echo htmlentities($responseXml).'<br />';
		$stoptime = time()+microtime();
		if ($stoptime-$starttime>60) exit;
	}
