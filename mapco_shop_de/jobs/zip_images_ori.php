<?php
	...
	$file=array();
	$file_id=array();
	$results4=q("SELECT * FROM cms_files WHERE original_id>0;", $dbweb, __FILE__, __LINE__);
	if (mysqli_num_rows($results4)>0)
	{
		while($row4=mysqli_fetch_array($results4))
		{
			$results=q("SELECT * FROM cms_files WHERE filename='".$row4["filename"]."' LIMIT 1;", $dbweb, __FILE__, __LINE__);
