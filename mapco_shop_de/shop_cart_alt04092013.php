	...
	if ((isset($_POST["form_button"]) || $onlinepaymentcheckout) and !isset($_POST["cartupdate"]) and !isset($_POST["cartclear"]))
	{
		...
		else
		{
			...
			if ($_SESSION["id_user"]!=30234)	// DEMO Kunde
			{
				...
				while ($row_codes=mysql_fetch_array($res_codes))
				{
			...
			*/	
			...
			//	if (isset($_SESSION["shippingtype_id"])) $shipping_type_id = $_SESSION["shippingtype_id"];
				...
				if (isset($_SESSION["id_shipping"]))
				{
					...
					if (mysql_num_rows($res_shippingtype_id)==0)
					{
						$shipping_type_id=0;
					}
					else
					{
						$row_shippingtype_id=mysql_fetch_array($res_shippingtype_id);
				...
				{
					$res_paymenttype_id=q("SELECT * FROM shop_payment WHERE id_payment = ".$_SESSION["id_payment"].";", $dbshop, __FILE__, __LINE__);
					if (mysql_num_rows($res_paymenttype_id)==0)
					{
						$payment_type_id=0;
					}
					else
					{
						$row_paymenttype_id=mysql_fetch_array($res_paymenttype_id);
				...
				*/
				...
				q("INSERT INTO shop_orders (shop_id, status_id, status_date, Currency_Code, customer_id, ordernr, comment, usermail, userphone, userfax, usermobile, bill_company, bill_gender, bill_title, bill_firstname, bill_lastname, bill_zip, bill_city, bill_street, bill_number, bill_additional, bill_country, bill_country_code, ship_company, ship_gender, ship_title, ship_firstname, ship_lastname, ship_zip, ship_city, ship_street, ship_number, ship_additional, ship_country, ship_country_code, shipping_costs, shipping_type_id, shipping_details, Payments_TransactionID, Payments_TransactionState, Payments_TransactionStateDate, Payments_Type, payments_type_id, PayPal_PendingReason, PayPal_BuyerNote, partner_id, bill_adr_id, ship_adr_id, firstmod, firstmod_user, lastmod, lastmod_user, username, password, shipping_net) VALUES(".$shop_id.", ".$status_id.", ".time().", 'EUR', '".$_SESSION["id_user"]."', '".mysql_real_escape_string($_SESSION["ordernr"], $dbshop)."', '".mysql_real_escape_string($_SESSION["comment"], $dbshop)."', '".mysql_real_escape_string($_SESSION["usermail"], $dbshop)."', '".mysql_real_escape_string($_SESSION["userphone"], $dbshop)."', '".mysql_real_escape_string($_SESSION["userfax"], $dbshop)."', '".mysql_real_escape_string($_SESSION["usermobile"], $dbshop)."', '".mysql_real_escape_string($_SESSION["bill_company"], $dbshop)."', '".mysql_real_escape_string($bill_gender, $dbshop)."', '".mysql_real_escape_string($_SESSION["bill_title"], $dbshop)."', '".mysql_real_escape_string($_SESSION["bill_firstname"], $dbshop)."', '".mysql_real_escape_string($_SESSION["bill_lastname"], $dbshop)."', '".mysql_real_escape_string($_SESSION["bill_zip"], $dbshop)."', '".mysql_real_escape_string($_SESSION["bill_city"], $dbshop)."', '".mysql_real_escape_string($_SESSION["bill_street"], $dbshop)."', '".mysql_real_escape_string($_SESSION["bill_number"], $dbshop)."', '".mysql_real_escape_string($_SESSION["bill_additional"], $dbshop)."', '".mysql_real_escape_string($_SESSION["bill_country"], $dbshop)."', '".mysql_real_escape_string($country_code[$_SESSION["bill_country"]], $dbshop)."', '".mysql_real_escape_string($_SESSION["ship_company"], $dbshop)."', '".mysql_real_escape_string($ship_gender, $dbshop)."', '".mysql_real_escape_string($_SESSION["ship_title"], $dbshop)."', '".mysql_real_escape_string($_SESSION["ship_firstname"], $dbshop)."', '".mysql_real_escape_string($_SESSION["ship_lastname"], $dbshop)."', '".mysql_real_escape_string($_SESSION["ship_zip"], $dbshop)."', '".mysql_real_escape_string($_SESSION["ship_city"], $dbshop)."', '".mysql_real_escape_string($_SESSION["ship_street"], $dbshop)."', '".mysql_real_escape_string($_SESSION["ship_number"], $dbshop)."', '".mysql_real_escape_string($_SESSION["ship_additional"], $dbshop)."', '".mysql_real_escape_string($_SESSION["ship_country"], $dbshop)."', '".mysql_real_escape_string($country_code[$_SESSION["ship_country"]], $dbshop)."', '".$_SESSION["shipping_costs"]."', ".$shipping_type_id.", '".mysql_real_escape_string($_SESSION["shipping_details"], $dbshop)."', '".mysql_real_escape_string($_SESSION["PaymentTransactionID"], $dbshop)."', '".mysql_real_escape_string($_SESSION["PaymentStatus"], $dbshop)."', ".$PaymentTransactionStateDate.", '".mysql_real_escape_string($onlinePaymentTyp, $dbshop)."', ".$payment_type_id.", '".mysql_real_escape_string($_SESSION["PayPalPendingReason"], $dbshop)."', '".mysql_real_escape_string($_SESSION["bill_PayPalNote"], $dbshop)."', ".$pid.", ".$_SESSION["bill_adr_id"].", ".$_SESSION["ship_adr_id"].", ".time().", ".$_SESSION["id_user"].", ".time().", ".$_SESSION["id_user"].", '".mysql_real_escape_string($row3["username"], $dbshop)."', '".mysql_real_escape_string($row3["password"], $dbshop)."', '".$_SESSION["shipping_net"]."');", $dbshop, __FILE__, __LINE__);
...
*/
	...
	while($row=mysql_fetch_array($results))
	{
//		if ($row["id_country"]==1) $selected="selected";
//		else $selected="";
		echo '<option '.$selected.' value="'.$row["id_country"].'">'.t($row["country"], __FILE__, __LINE__).'</option>';
	}
	echo '			</select>';
	echo '		</td>';
	echo '	</tr>';
