...
?>
	...
	{
		$.post("<?php echo PATH ?>soa/", { API:"ebay_lms", Action:"startDownloadJob", id_account:$id },
			function($data)
			{
				show_status2($data);
				return;
...
//					show_status2($data);
		...
		{
			$("#view_categories").html('<img src="<?php echo PATH ?>images/icons/loaderb64.gif" style="margin:0px 10px 0px 30px" alt="Bitte warten!" /><br />Lese Kategorien bei eBay aus...');
			$.post("<?php echo PATH ?>soa/", { API:"ebay", Action:"GetCategories", id_account:$id_account },
				function(data)
				{
					...
					$xml.find("Category").each(
						function()
						{
							...
							$CategoryLevel=$(this).find("CategoryLevel").text();
							$CategoryName=$(this).find("CategoryName").text();
							$ebay_categories[$CategoryID]=$CategoryName;
							for($i=0; $i<$CategoryLevel; $i++) $CategoryName='&nbsp;&nbsp;'+$CategoryName;
		...
		function category_edit_save()
		{
			...
			var $CategoryID = $("#category_edit_CategoryID").val();
			var $CategoryID2 = $("#category_edit_CategoryID2").val();
			$.post("<?php echo PATH; ?>soa/", { API:"ebay", Action:"CategoriesEdit", id_account:$id_account, GART:$GART, CategoryID:$CategoryID, CategoryID2:$CategoryID2 },
				function(data)
				{
					try
					{
...
//			$("#view_accounts").html('<img src="<?php echo PATH ?>images/icons/loaderb64.gif" style="margin:0px 0px 0px 30px" alt="Bitte warten!" />');
				...
				function(data)
				{
					...
					$xml.find("Account").each(
						function()
						{
							...
							$listhtml += '	</div>';
							$listhtml += '	<div style="width:238px;">';
							$listhtml += '		<img src="<?php echo PATH; ?>images/icons/24x24/down.png" alt="Bestellungen herunterladen" title="Bestellungen herunterladen" onclick="start_download('+$Id+');" />';
							$listhtml += '		<img src="<?php echo PATH; ?>images/icons/24x24/search.png" alt="Auktionen abgleichen" title="Auktionen abgleichen" onclick="account_check('+$Id+', 1, 0);" />';
							$listhtml += '	</div>';
							$listhtml += '</li>';
						}
					);
		...
		function view_categories()
		{
			...
			$.post("<?php echo PATH ?>soa/", { API:"ebay", Action:"CategoriesGet", id_account:$id_account },
				function(data)
				{
								...
								function()
								{
									...
									$categories.find("Category").each(
										function()
										{
											...
											{
												$CategoryID=$(this).find("CategoryID").text();
												$CategoryID2=$(this).find("CategoryID2").text();
