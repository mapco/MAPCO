...
//#########################################################################################################################
	...
	function draw_actions_menu(orderid)
	{
		...
		{
			menu+='<small>IDIMS-Auftragb<br />bereits erfasst</small><br /><br />';
		}
//		menu+='<a href="javascript:order_send('+orderid+');">IDIMS</a><br /><br />';
		...
		//FAHRZEUGANFRAGEMAIL NUR BEI EBAY-SHOPS
		/*
		if (Shop_Shops[orders[orderid]["platformID"]]["shop_type"]==2)
		{
			var vehicle_count=0;
			...
			//FAHRZEUGABFRAGE NUR WENN KEINE DATEN VORHANDEN && AUFTRAG NOCH NICHT GESCHRIEBEN
			if (vehicle_count==0 && (orders[orderid]["status_id"]==1 || orders[orderid]["status_id"]==7))
			{
				menu+='	<li style="margin:5px; margin-left:-22px;"><a href="javascript:send_fin_mail_single_dialog('+orderid+')">Fahrzeugdaten anfragen</a></li>';
			}
		}
		*/
		...
		menu+='	<li style="margin:5px; margin-left:-22px;"><a href="javascript:set_order_payment('+orderid+');">Bezalt markieren</a></li>';
	...
	function show_platform_data(orderid)
	{
		...
		html+=Shop_Shops[orders[orderid]["platformID"]]["title"];
	...
	function show_order_sum(orderid)	
	{
		...
		var shippingtype='';
		/*
		if (shipping_details.search("DHL")>0 || shipping_details.search("E_DHLPaketInternational")>0) shippingtype='<span style="background-color:yellow;"><b>DHL</b></span>';
		if (shipping_details.search("DPD")>0 || shipping_details.search("E_PaketInternational")>0) shippingtype='<span style="background-color:red;"><b>DPD</b></span>';							
		*/
	...
	function view_box()
	{
		...
		wait_dialog_show();
		$.post("<?php echo PATH; ?>soa/", { API: "crm", Action: "crm_orders_list", FILTER_Platform:FILTER_Platform, FILTER_Status:FILTER_Status, FILTER_SearchFor:FILTER_SearchFor, FILTER_Searchfield:FILTER_Searchfield, OrderBy:OrderBy, OrderDirection:OrderDirection, ResultPage:ResultPage,ResultRange:ResultRange, date_from:date_from, date_to:date_to},
			function(data)
			{
			//show_status2(data);
				ResultPage=1;
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	function set_order_payment(orderid)
	{
		...
		//SHOPS AUßER EBAY
		if (Shop_Shops[orders[orderid]["platformID"]]["shop_type"]!=2)
		//if (orders[orderid]["platformID"]!=3 && orders[orderid]["platformID"]!=4)
		{
			for (var arrayIndex in PaymentTypes)
	//		  fourth += ' ' + person[arrayIndex];

	//		PaymentTypes.forEach(function (key, val)
			{
	...
	//	parentorderid=1749450;
		...
		*/
		...
		if (Shop_Shops[shop_id]["shop_type"]==2)
		{
			...
			$.post("<?php echo PATH; ?>soa2/index.php", { API: "shop", APIRequest: "OrderAdd", mode:"copy", id_order:parentorderid, foreign_OrderID:"", combined_with:combined_with},
				function(data)
				{
					//alert(data);
					var $xml=$($.parseXML(data));
...
//	echo '			<option value="7">PayPal-Transaction-ID</option>';
	...
	{
		echo '		<input type="text" size="25" id="FILTER_Searchfield" value="'.$_GET["FILTER_Searchfield"].'" />';
	}
	else
	{
		echo '		<input type="text" size="25" id="FILTER_Searchfield" />';
	}
	echo '	</span>';
	echo '	<span class="tabs" id="FILTER_Date" style="width:350px; text-align:center">';
	echo '		Datum von <input type="text" id="date_from" size="10" onchange="date_from=this.value;" />';
	echo '		&nbsp;bis&nbsp;<input type="text" id="date_to" size="10" onchange="date_to=this.value;" />';
	echo '	</span>';
	echo '	<span>';
