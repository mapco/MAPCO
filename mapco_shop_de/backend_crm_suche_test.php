...
var view_mode="<?php if (isset($_GET["view_mode"])) echo $_GET["view_mode"]; ?>";
alert(view_mode);
	...
	function show_customer_table(data)
	{
		...
		if (view_mode=="")
		{
			...
			table+='		<img style="margin:0px 5px 0px 0px; border:0; padding:0; cursor:pointer; float:right;" src="images/icons/24x24/add.png" alt="Kunde(n) zu einer Liste hinzufügen" title="Kunde(n) zu einer Liste hinzufügen" onclick="add_customer_to_costumer_list();" />';
		}
		table+='	</th>';
		table+='</tr>';
		...
		function()
		{
			...
			if (view_mode=="order_customer_search")
			{
				table+='<a href="<?php echo PATH."backend_crm_orders_details.php?OrderID=".$_GET["OrderID"]; ?>&crm_userID='+customer_id+'"><img style="margin:0px 5px 0px 0px; border:0; padding:0; cursor:pointer; float:right;" src="images/icons/24x24/edit.png" alt="Kunde zur Bestellung übernehmen" title="Kunde zur Bestellung übernehmen" /></a>';
			}
			table+='	</td>';
			table+='</tr>';
...
//				alert(data);
	...
	function show_searchbox()
	{
		...
		//	$(".reminder").css("font-weight", "normal");
