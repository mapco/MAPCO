<?php
	...
	echo $crm_userID=$_GET["crm_userID"];
	//$OrderID=1728099;
?>
...
var crm_userID=<?php echo $crm_userID; ?>;
				...
				if (Ack=="Success") 
				{
					...
					$xml.find("vehicle").each(
						function()
						{
							...
							table+='<tr ondblclick="select_customer_vehicle('+customer_vehicle_id+');" style="background-color:#fff; cursor:pointer">';
							table+='	<td>';
		...
		$.post("<?php echo PATH; ?>soa/", { API: "crm", Action: "crm_get_customer_vehicles", vehicle_customer_id:customer_vehicle_id, mode:"vehicle"},
			function(data)
			{
				...
				if (Ack=="Success") 
				{
					...
					$("#update_vehicle_dateBuiltMonth").val(convert_time_from_timestamp($(this).find("vehicleDateBuilt").text(),"month"));
					$("#update_vehicle_dateBuiltYear").val(convert_time_from_timestamp($(this).find("vehicleDateBuilt").text(),"year"));
	...
	function update_customer_vehicle()
	{
		...
		wait_dialog_show();
		$.post("<?php echo PATH; ?>soa/", { API: "crm", Action: "crm_update_orderItem_vehicle", mode:mode, vehicleID:vehicleID, customerVehicleID:customerVehicleID, crm_customer_id:crm_customer_id, HSN:HSN, TSN:TSN, dateBuilt:dateBuilt, FIN:FIN, additional:additional },
			function(data)
			{
				wait_dialog_hide();
				var $xml=$($.parseXML(data));
	...
	function vehicle_item_update(id)
	{
		if (id.search("orderposition_HSN_")>-1)
		{
					...
					$xml.find("Order").each(
						function()
						{
							...
							function()
							{
								...
								orderstable+='	</td>';
								orderstable+='	<td id="orderposition_HSN_'+orderItemID+'" ondblclick="vehicle_item_update(this.id);" style="cursor:pointer">';
								orderstable+=$(this).find("OrderItemVehicleHSN").text();
								orderstable+='	</td>';
								orderstable+='	<td id="orderposition_TSN_'+orderItemID+'" ondblclick="vehicle_item_update(this.id);" style="cursor:pointer">';
								orderstable+=$(this).find("OrderItemVehicleTSN").text();
								orderstable+='	</td>';
								orderstable+='	<td id="orderposition_datebuilt_'+orderItemID+'" ondblclick="vehicle_item_update(this.id);" style="cursor:pointer">';
								orderstable+=convert_time_from_timestamp($(this).find("OrderItemVehicleDateBuilt").text(),"datebuilt");
								orderstable+='	</td>';
								orderstable+='	<td id="orderposition_FIN_'+orderItemID+'" ondblclick="vehicle_item_update(this.id);" style="cursor:pointer">';
								orderstable+=$(this).find("OrderItemVehicleFIN").text();
								orderstable+='	</td>';
								orderstable+='	<td id="orderposition_additionalcardata_'+orderItemID+'" ondblclick="vehicle_item_update(this.id);" style="cursor:pointer">';
	...
	{
		var href="";
		href+='<?php echo PATH."backend_crm_suche_test.php?"; ?>';
		href+='OrderID='+OrderID;
		href+='&view_mode=order_customer_search';
	alert(href);
...
<?php	
	...
	echo '<p><a href="">Zurück zur Bestellungsübersicht</a></p>';
	...
	echo '			</span>';
	echo '			<span style="width:248px; float:left;" id="BuyerData_Address_Gender"></span>';
	echo '			<span style="width:248px; float:left;" id="BuyerData_Address_Name"></span>';
	echo '			<span style="width:248px; float:left;" id="BuyerData_Address_Street"></span>';
	echo '			<span style="width:248px; float:left;" id="BuyerData_Address_Additional"></span>';
	echo '			<span style="width:50px; float:left;" id="BuyerData_Address_Zip"></span>';
	echo '			<span style="width:198px; float:left;" id="BuyerData_Address_City"></span>';
	echo '			<span style="width:248px; float:left;" id="BuyerData_Address_Country"></span>';
	echo ' 		</div>';
	...
	echo '<tr>';
	echo '	<td>KBA</td>';
	echo '	<td><input type="text" size="4" id="update_vehicle_HSN" />&nbsp;/&nbsp;<input type="text" size="4" id="update_vehicle_TSN" />&nbsp;<button id="btn1" onclick="get_vehicle_byKBA();">Fahrzeug suchen</button></td>';
	echo '<tr>';
