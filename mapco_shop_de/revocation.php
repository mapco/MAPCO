...
//	exit;
	...
	else
	{
		...
		//shopitems
		elseif ( startsWith($_GET["url"], "ersatzteil/") )
		{
			//get id_item
		...
		//shop_item_vehicles.php
		elseif ( startsWith($_GET["url"], "ersatzteil-fahrzeuge/") )
		{
			//get id_item
			...
			if ( is_numeric($_GET["id_item"]) )
			{
				...
				$cut=substr($_GET["url"], 21, 1000);
				$title=stripslashes(substr($cut, strpos($cut, "/")+1, 1000));
				include('shop_item_vehicles.php');
		...
		elseif ( $_GET["url"]!="news/" and startsWith($_GET["url"], "news/") )
		{
			...
			elseif ( $_GET["id_article"]=="der-schrauber" )
			{
				//get title
