...
{	
	...
	{ 
		...
		$(".button_remove_label").click(function(){
			alert($( this ).attr( 'value' ));//remove_label($( this ).attr( 'value' ) );
		});
...
var message = 'Übersetzung speichern?';
	...
	$callback = function(){
		...
		var published = $("#translation_published").val();
		...
		{ 
			...
			if ( translation_id == 0)
			{
				$("#translation_tab_"+language_id).attr('value',$xml.find('insert_id').text());
			}
		}); 
	}
	dialog_confirm(message,$callback);
...
{ 
	...
	translations[$active_language]['article_introduction'] = $("#article_introduction").val();
	translations[$active_language]['article_text'] = $("#article_text").val();

	var article_id = $("#editor_tabs").attr('article');
	var	l_translation_id = translation_id;
	...
	var $callback=function(){
		...
		{ 
			...
			$("#article_introduction").val(translations[language_id]['article_introduction']);
			...
			$("#Language_tabs").attr('active_translation', l_translation_id);
