...
?>
	...
	function images_upload_file($filenr, $pos)
	{
		...
		if( $pos>=$files[$filenr].size )
		{
			...
			$postdata["API"]="cms";
			$postdata["Action"]="ContactGet";
			$postdata["id_contact"]=id_contact;
			$.post('<?php echo PATH; ?>soa/', $postdata, function($data)
			{
				...
				if( $images_upload_id_article=="" || $images_upload_id_article==0 )
				{
					show_status("Es konnte kein Beitrag zum angegebenen Kontakt gefunden werden.");
					return;
				}
				...
				var $postdata=new Object();
				$postdata["API"]="cms";
				$postdata["Action"]="ArticleImageAdd";
				$postdata["id_article"]=$images_upload_id_article;
				$postdata["filename"]=$filename;
				$postdata["source"]=$tempfile;
				$.post('<?php echo PATH; ?>soa/', $postdata, function($data)
				{
					try { $xml = $($.parseXML($data)); } catch ($err) { show_status2($err.message); return; }
					$ack = $xml.find("Ack");
					...
					$images_upload_imageformats=new Array(23, 24);
	...
	function images_upload_file_imageformats($filenr, $pos)
	{
		...
		$postdata["id_article"]=$images_upload_id_article;
		$postdata["id_file"]=$images_upload_id_original;
		$postdata["id_imageformat"]=$images_upload_imageformats[$pos];
		$.post('<?php echo PATH; ?>soa/', $postdata, function($data)
		{
			try { $xml = $($.parseXML($data)); } catch ($err) { show_status2($err.message); return; }
			$ack = $xml.find("Ack");
	...
	function image_mass_upload_fileDialogComplete(numFilesSelected, numFilesQueued)
	{
		...
		$.post("<?php echo PATH; ?>soa/", { API:"shop", Action:"ListAdd", title:"Bilderupload "+today, id_listtype:2 }, function($data)
		{
			...
			$.post("<?php echo PATH; ?>soa/", { API:"shop", Action:"ListFieldAdd", list_id:id_list, field_id:19 }, function($data)
			{
				view();
