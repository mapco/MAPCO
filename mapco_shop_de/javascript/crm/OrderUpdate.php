...
*/
	...
	function order_add()
	{
		...
		{
			if (ordertype_id!=4 && ordertype_id!=6)
			{
				if (ordertype_id==2)
				{
				...
				[
					{ text: "Bestellung anlegen", click: function() { 
						...
						if ( $("#order_add_ordertype").val() == 5 )
						{
							...
							wait_dialog_show();
							$.post("<?php echo PATH; ?>soa2/index.php", { API: "shop", APIRequest: "OrderAdd", mode:"manual", Currency_Code:currency, shop_id:shop_id, ordertype_id:$("#order_add_ordertype").val(), status_id:$status_id},
								function($data)
								{
									//alert ($data);
									wait_dialog_hide();
...
//		$html +=		'<option value="Freitext">Freitextsuche</option>';
	...
	function change_orderVAT_dialog(orderid)
	{
		...
		if (orders[orderid]["shop_id"] == 1 || orders[orderid]["shop_id"] == 3 || orders[orderid]["shop_id"] == 5 || orders[orderid]["shop_id"] == 7 || orders[orderid]["shop_id"] == 22)
		{
		...
		else
		{
			...
			if (orders[orderid]["VAT"]==0) var selected = 'selected'; else var selected ='';
			html+='		<option value=0 '+selected+'>0 %</option>';
			if (orders[orderid]["shop_id"] == 2 || orders[orderid]["shop_id"] == 4 || orders[orderid]["shop_id"] == 6) // AUTOPARTNER
			{
				if (orders[orderid]["bill_country_code"] == 'AT')
				{
					if (orders[orderid]["VAT"]==20) var selected = 'selected'; else var selected ='';
