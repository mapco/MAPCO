...
//			echo '<br /><br />'.htmlentities($datapost["FeedContent"]).'<br /><br />';
	...
	$results=q("SELECT * FROM amazon_products WHERE account_id=1 ORDER BY lastquantityupdate LIMIT 50000;", $dbshop, __FILE__, __LINE__);
	while( $row=mysql_fetch_array($results) )
	{
		...
		if( mysql_num_rows($results2)==0 ) $Quantity=0;
		else
		{
			$row2=mysql_fetch_array($results2);
			$Quantity=$row["ISTBESTAND"]+$row["MOCOMBESTAND"];
		}
