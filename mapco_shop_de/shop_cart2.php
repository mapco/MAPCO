...
<?php
	...
	if (isset($_POST["Userdata"]) && $_POST["Userdata"]=="paygenic")
	{
	...
	elseif ($onlinePayment["method"]=="PayPal")
	{
		$onlinepaymentcheckout=false;
	}
	
	//ZWEITER (+DRITTER) PAYPAL-DURCHLAUF ->GetPayPalCheckout (+DoPayPalCheckout)
	if ($onlinePayment["selected"] && $onlinePayment["method"]=="PayPal" && $onlinepaymentOK && !$onlinepaymentcheckout) {
	...
	if ((isset($_POST["form_button"]) || $onlinepaymentcheckout) and !isset($_POST["cartupdate"]) and !isset($_POST["cartclear"]))
	{
		...
		else
		{
			...
			while($row=mysqli_fetch_array($results))
			{
				...
				q("INSERT INTO shop_orders_items3 (order_id, item_id, amount, price, netto) VALUES('".$order_id."', '".$row["item_id"]."', ".$row["amount"].", '".$price["total"]."', '".$price["net"]."');", $dbshop, __FILE__, __LINE__);
			}
			...
			unset($_SESSION["PaymentPaymentStatus"]);
