<?php
	...
	else
	{
		...
		elseif (startsWith($_GET["url"], "regional-center/"))
		{
			...
			$results=q("SELECT * FROM cms_locations WHERE location='mapco-".$domain.".de' LIMIT 1;", $dbweb, __FILE__, __LINE__);
			if (mysql_num_rows($results)==1)
			{
				$row=mysql_fetch_array($results);
				$_GET["rcid"]=$row["RC_NR"];
				$_GET["pid"]=$row["PID"];
				$_GET["rcbez"]=ucfirst($domain);
			}
			include('shop.php');
		}
		...
		{
			$results=q("SELECT * FROM cms_menuitems WHERE alias='".mysql_real_escape_string($_GET["url"], $dbweb)."' LIMIT 1;", $dbweb, __FILE__, __LINE__);
			if (mysql_num_rows($results)>0)
			{
				$row=mysql_fetch_array($results);
