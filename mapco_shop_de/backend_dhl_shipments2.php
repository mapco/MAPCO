...
//			window.frames["printframe"].focus();
		...
		function label_create()
		{
			...
			//check for packstation in small letters
				...
				function($data)
				{
					...
					$.post("<?php echo PATH ?>soa/", { API:"dhl", Action:"GetShipperAddress" }, function($data)
					{
						...
						if( $ship_adr_id>0 )
						{
							...
							if( $("#order_ReceiverCompany").val() == "" ) $("#order_ReceiverCompany").val( $.trim( $.trim($xml.find("ship_firstname").text())+" "+$.trim($xml.find("ship_lastname").text()) ) ) 
						...
						else
						{
							...
							if( $("#order_ReceiverCompany").val() == "" ) $("#order_ReceiverCompany").val( $.trim( $.trim($xml.find("bill_firstname").text())+" "+$.trim($xml.find("bill_lastname").text()) ) ) 
				...
				if( code == 13 )
				{
					if( $needle.substr(0, 1)=="*" )
					{
						...
						$.post("<?php echo PATH; ?>soa/", { API:"idims", Action:"AddressGet", "search":$needle },	function($data)
						{
							...
							show_location_label($company1, $company2, $street, $streetnr, $xml.find("PLZ").text(), $xml.find("ORT").text(), $xml.find("ANSCHR_3").text(), 'DE', '', '');
						});
					}
