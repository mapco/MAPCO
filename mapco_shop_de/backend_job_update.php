...
<script language="javascript">
	...
	function item_create(i)
	{
		...
		*/
		...
		$("#items_submit_dialog").html('Account: '+id_account+'<br /><br /><a href="http://www.mapco.de/backend_shop_item_editor.php?id_item='+items[i]+'" target="_blank">Shopartikel</a> '+(i+1)+" von "+items.length+"<br /><br />Erstelle Auktionen...");
		var response = $.post("<?php echo PATH; ?>soa/", { API:"ebay", Action:"ItemCreateAuctions", id_account:id_account, id_item:items[i], bestoffer:bestoffer, ShippingServiceCost:ShippingServiceCost, id_article:id_article[id_account], id_imageformat:id_imageformat, items_submit_skiphours:items_submit_skiphours },
			function(data)
			{
				try
				{
...
//		var items_submit_skiphours=$("#items_submit_skiphours").val();
		var status='Account: '+id_account+'<br /><br /><a href="http://www.mapco.de/backend_shop_item_editor.php?id_item='+items[item_counter]+'" target="_blank">Shopartikel</a> '+(item_counter+1)+" von "+items.length;
...
/*
	//standard routine
	$results=q("SELECT * FROM shop_items WHERE lastupdate<".mktime(16, 00, 00, 03, 14, 2013)." AND lastupdatestart<".(time()-3600)." ORDER BY lastupdate LIMIT 1;", $dbshop, __FILE__, __LINE__);
	while( $row=mysql_fetch_array($results) )
	{
...
*/
	...
	$results=q("SELECT shopitem_id FROM ebay_auctions WHERE account_id=2 AND Description LIKE '%Selbstabholung%' GROUP BY shopitem_id LIMIT 1;", $dbshop, __FILE__, __LINE__);
...
*/
    ...
    echo '</script>';
	echo '<span style="color:#ffffff; font-size:40px;">ToDo: '.$todo.'</span>';
	if($todo==0)
	{
