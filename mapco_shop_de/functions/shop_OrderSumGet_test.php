<?php
	...
	*/
	...
	define('TABLE_SHOP_ORDERS_RETURNS', 'shop_returns2');
	define('TABLE_SHOP_ORDERS_RETURNS_ITEMS', 'shop_returns_items');
	define('TABLE_SHOP_ORDERS_CREDITS', 'shop_orders_credits2');
	define('TABLE_SHOP_ORDERS_CREDITS_POSITIONS', 'shop_orders_credits_positions');
	...
	// G E T   D A T A ===============================================================================================
		...
		if (sizeof($order_id_list)>0)
		{
			$res_credits = q("SELECT * FROM ".TABLE_SHOP_ORDERS_CREDITS." WHERE order_id IN (".implode(",", $order_id_list).")", $dbshop, __FILE__, __LINE__);
			while($row_credits = mysqli_fetch_assoc($res_credits))
			{
				$credits[$row_credits['id_shop_order_credit']] = $row_credits;
			
				$credits_ids[] = $row_credits['id_shop_order_credit'];
			}
		}
		
		//CREDITS POSITIONS
		$credits_positions = array();
		if ( sizeof($credits_ids) > 0 )
		{
			$res_credits_positions = q("SELECT * FROM ".TABLE_SHOP_ORDERS_CREDITS_POSITIONS." WHERE order_credit_id IN (".implode(", ", $credits_ids).")", $dbshop, __FILE__, __LINE__);
			while ( $row_credits_positions = mysqli_fetch_assoc( $res_credits_positions ) )
			{
				if (isset($credits_positions[$row_credits_positions["order_credit_id"]]))
				{
					$credits_positions[$row_credits_positions["order_credit_id"]] [sizeof( $credits_positions[$row_credits_positions["order_credit_id"]] )] = $row_credits_positions;
				}
				else
				{
					$credits_positions[$row_credits_positions["order_credit_id"]] [0] = $row_credits_positions;
				}
			}
		}
	...
	//=================================================================================================================
		if ( $params["calculation_base"] == "net" )
		{
			...
			{
				$credit_net = 0;
				if ( isset ( $credits_positions[$credit_id]) )
				{
					$credit_position_count = 0;
					foreach ( $credits_positions[$credit_id] as $index_pos => $creditposition )
					{
						$credit_net += $creditposition['net'];
						$credit_position_count ++;
					}
					
					$ex = $credit["exchange_rate_from_EUR"];

					$response["OrderCredits"] [$credit_id] ["NetEUR"] 		+= $credit_net;
					$response["OrderCredits"] [$credit_id] ["GrossEUR"]		+= $credit_net * $VAT_m;
					$response["OrderCredits"] [$credit_id] ["TaxEUR"]		+= $credit_net * ( $VAT_m-1 );
					$response["OrderCredits"] [$credit_id] ["NetFC"] 		+= $credit_net * $ex;
					$response["OrderCredits"] [$credit_id] ["GrossFC"] 		+= $credit_net * $ex * $VAT_m;
					$response["OrderCredits"] [$credit_id] ["TaxFC"] 		+= $credit_net * $ex * ( $VAT_m-1 );
					$response["OrderCredits"] [$credit_id] ["ItemsCount"]	= $credit_position_count;
					$response["OrderCredits"] [$credit_id] ["Currency_Code"]= $credit[$credit_id]["Currency_Code"];
					$response["OrderCredits"] [$credit_id] ["ExchangeRate"]	= $ex;
					...
					$response["OrderCredits"] ["ItemsCount"]	= $credit_position_count;
					$response["OrderCredits"] ["Currency_Code"]	= $credit[$credit_id]["Currency_Code"];
				}
			}
			...
			*/
			...
			$response["OrderTotalWithCredit"] ["NetFC"] 		= $response["OrderTotal"] ["NetFC"] 	- $response["OrderCredits"] ["NetEUR"];
			$response["OrderTotalWithCredit"] ["GrossFC"] 		= $response["OrderTotal"] ["GrossFC"] 	- $response["OrderCredits"] ["GrossFC"];
			$response["OrderTotalWithCredit"] ["TaxFC"] 		= $response["OrderTotal"] ["TaxFC"] 	- $response["OrderCredits"] ["TaxFC"];
			$response["OrderTotalWithCredit"] ["NetEUR"] 		= $response["OrderTotal"] ["NetEUR"] 	- $response["OrderCredits"] ["NetEUR"];
			$response["OrderTotalWithCredit"] ["GrossEUR"] 		= $response["OrderTotal"] ["GrossEUR"] 	- $response["OrderCredits"] ["GrossEUR"];
			$response["OrderTotalWithCredit"] ["PositionsCount"]= $response["OrderPositionsTotal"] ["PositionsCount"];;
			$response["OrderTotalWithCredit"] ["ItemsCount"] 	= $response["OrderPositionsTotal"] ["ItemsCount"];;
			$response["OrderTotalWithCredit"] ["Currency_Code"] = $response["OrderTotal"] ["Currency_Code"];
		...
		if ( $params["calculation_base"] == "gross" )
		{
			...
			foreach ( $credits as $credit_id => $credit )
			{
				...
				if ( isset ( $credits_positions[$credit_id]) )
				{
					...
					foreach ( $credits_positions[$credit_id] as $index_pos => $creditposition )
					{
						$credit_gross += $creditposition['gross'];
						$credit_position_count ++;
					}
					...
					$response["OrderCredits"] [$credit_id] ["NetEUR"] 		+= $credit_gross / $VAT_m;
					$response["OrderCredits"] [$credit_id] ["GrossEUR"]		+= $credit_gross;
					$response["OrderCredits"] [$credit_id] ["TaxEUR"]		+= $credit_gross / ($VAT_m * 100 ) * ($VAT_m*100-100);
					$response["OrderCredits"] [$credit_id] ["NetFC"] 		+= $credit_gross / $ex / $VAT_m;
					$response["OrderCredits"] [$credit_id] ["GrossFC"] 		+= $credit_gross / $ex;
					$response["OrderCredits"] [$credit_id] ["TaxFC"] 		+= $credit_gross / $ex / ($VAT_m * 100 ) * ($VAT_m*100-100);
					$response["OrderCredits"] [$credit_id] ["ItemsCount"]	= $credit_position_count;
					$response["OrderCredits"] [$credit_id] ["Currency_Code"]= $credit[$credit_id]["Currency_Code"];
					$response["OrderCredits"] [$credit_id] ["ExchangeRate"]	= $ex;
