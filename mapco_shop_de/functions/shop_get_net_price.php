<?php
	///äöüÄÖÜ UTF-8
	if (!function_exists("get_net_price"))
	{
		function get_net_price($id_item, $amount=1, $customer_id=0)
		{
			global $dbweb;
			global $dbshop;
...
//							if ($discount>0) $pl[1]*=(100-$discount)/100;
	...
	//			echo '4: '.$pl[3].'<br />';
			...
			{
				//neue Werksverkaufsliste
				$results2=q("SELECT * FROM prpos WHERE ARTNR='".$artnr."' AND LST_NR='2';", $dbshop, __FILE__, __LINE__);
				if (mysql_num_rows($results2)>0)
				{
					$row2=mysql_fetch_array($results2);
					if ($row2["POS_0_WERT"]>0)
				...
				{
					//alternativ blaue Liste
					$results3=q("SELECT * FROM prpos WHERE ARTNR='".$artnr."' AND LST_NR='3';", $dbshop, __FILE__, __LINE__);
					if (mysql_num_rows($results3)>0)
					{
						$row3=mysql_fetch_array($results3);
						if ($row3["POS_0_WERT"]>0)
						{
							if ($amount>=$row3["POS_0_PE"])
							{
								$pl[0]=$row3["POS_0_WERT"];
								if ($discount>0) $pl[0]*=(100-$discount)/100;
								if ($pl[0]<=0) $pl[0]="9999";
								return(($pl[0]+$atwert));
							}
