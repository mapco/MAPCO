<?php
	...
	///äöüÄÖÜ UTF-8
	if (!function_exists("mail_order"))
	{
		function mail_order($id_order)
		{
			global $dbweb;
			global $dbshop;
		...
		//	$gewerblich=gewerblich($_SESSION["id_user"]);
			...
			$text1 .= '	<br>Eigene Bestellnummer: <b>'.$row["ordernr"].'<b>';
			...
			$text2 .= '<p>Eigene Bestellnummer: '.$row["ordernr"].'<br />';
			$text2 .= 'Anmerkung:<br />'.nl2br($row["comment"]).'</p>';
			...
			$results2=q("SELECT a.price, a.amount, c.title, b.MPN, b.id_item FROM shop_orders_items AS a, shop_items AS b, shop_items_de AS c WHERE a.order_id=".$row["id_order"]." AND a.item_id=b.id_item AND b.id_item=c.id_item;", $dbshop, __FILE__, __LINE__);
			$total=0;
...
*/
			...
			$text4 .= '<span style="font-size:18px;"><strong>'.t("Dies ist nur eine Bestellbestaetigung. Zahlung erst nach Erhalt der Rechnung durchfuehren!").'</font></span></p>';
			$text4 .= '</td></tr>';
			$text4 .= '<tr>';
			...
			*/
			...
			if ($customer_id=="358" or $customer_id=="22044" or $customer_id=="28625")
			{
				...
				send_html_mail_ma("developer@mapco.de", "Neue Bestellung im Online-Shop", $text1.$text.$text3);
			}
			else
			{
				send_html_mail($usermail, "Eingangsbestätigung zu Ihrer Bestellung", $text2.$text.$text4.$text5);
//				send_html_mail_ma("pm@mapco.eu", "Neue Bestellung im Online-Shop", $text1.$text.$text3);
...
*/
				...
				//Mail nach Borkheide
				send_html_mail_ma("bestellung@mapco-shop.de", "Neue Bestellung im Online-Shop", $text1.$text.$text3);
				//send_html_mail("developer@mapco.de", "Neue Bestellung im Online-Shop", $text1.$text);
				}
			}
